...
export class WppAutocomplete implements BaseComponent, InlineMessage {
  ...
  private _locales: AutocompleteLocales = LOCALES_DEFAULTS
   ...
   */
  @Prop() readonly locales: Partial<AutocompleteLocales> = {}
  ...
  @Watch('locales')
  onUpdateLocales(newLocales: Partial<AutocompleteLocales>) {
  ...
  private handleClickOutside(event: Event) {
    if (!isEventTargetContained(this.host, event) && this.isInComponent) this.isInComponent = false
  }
      ...
      return (
        <Fragment>
          ...
          {this.componentSuggestions?.map((suggestion, index) => {
            ...
            return (
              <wpp-list-item
                key={this.getOptionId(suggestion)}
                selectable={true}
                checked={isChecked}
                value={suggestion}
          ...
          >
            ...
            <div class="show-more-action">
              {this.showMoreLessRender(
                `+${this.activePillsTruncationState.filter(x => x).length} ${this._locales.showMore}`,
              )}
            </div>
          </div>
        )}
        {!this.isShowMore && <div class="show-less-action">{this.showMoreLessRender(this._locales.showLess)}</div>}
  ...
  private renderPillComponent = (
    ...
    const labelText =
      option.label ||
      option.slots?.find((slot: any) => slot.type === 'span' && slot.props.slot === 'label')?.props.children
                  ...
                  <div class="actions-container">
                    <wpp-list-item onClick={this.handleCreateNewOptionClick}>
                      <wpp-typography type="s-strong" class="create-new-option" slot="label">
                        {this._locales.createNewElement}
                      </wpp-typography>
                    </wpp-list-item>
                  </div>
                </div>
