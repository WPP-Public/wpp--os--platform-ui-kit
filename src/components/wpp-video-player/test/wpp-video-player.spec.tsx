...
import { CaptionValue } from '../types'

// Define an interface for the video mock to improve type safety
interface VideoElementMock {
  play: jest.Mock
  pause: jest.Mock
  muted: boolean
  autoplay: boolean
  ...
  textTracks: {
    length: number
    onaddtrack: null | ((event: any) => void)
  }
  buffered: {
    length: number
...
let originalQuerySelector: typeof HTMLElement.prototype.querySelector
let originalQuerySelectorAll: typeof HTMLElement.prototype.querySelectorAll
...
describe('wpp-video-player', () => {
  ...
  let pauseMock: jest.Mock<void>
  let videoMock: VideoElementMock
  ...
  beforeEach(() => {
    ...
    videoMock = {
      ...
      timeFormat: 'default',
      getAttribute: (attr: string) => (attr === 'autoplay' ? '' : null),
      hasAttribute: (attr: string) => attr === 'autoplay',
    ...
    WppVideoPlayer.prototype.componentDidLoad = function () {
      ...
      this['videoPlayerRef'] = videoMock as unknown as HTMLVideoElement
    }

    // Also mock querySelector in case it's used internally
    HTMLElement.prototype.querySelector = jest.fn().mockImplementation((selector: string) => {
    ...
    HTMLElement.prototype.querySelectorAll = jest.fn().mockImplementation((selector: string) => {
      if (selector === 'source') {
        return [{ getAttribute: (attr: string) => (attr === 'type' ? 'video/mp4' : null) }]
      }
      if (selector === 'track') {
        return [{ getAttribute: (attr: string) => (attr === 'srclang' ? 'en' : null) }]
      }
  ...
  describe('snapshots', () => {
    it('should render default component', async () => {
      const page = await newSpecPage({
        ...
        template: () => <wpp-video-player src={videoSrc}></wpp-video-player>,
      })
    ...
    it('should render component with autoplay and mute', async () => {
      const page = await newSpecPage({
        components: [WppVideoPlayer],
        template: () => <wpp-video-player src={videoSrc} controlPanelConfig={{ autoplay: true }}></wpp-video-player>,
      })
  ...
  describe('component', () => {
    describe('rendering', () => {
      ...
      it('should render with custom size', async () => {
        const page = await newSpecPage({
          components: [WppVideoPlayer],
          template: () => <wpp-video-player src={videoSrc} size={{ width: 800, height: 600 }}></wpp-video-player>,
        })
      ...
      it('should render with autoplay configuration', async () => {
        ...
        const video = page.root?.shadowRoot?.querySelector('video') as HTMLVideoElement
      ...
      it('should render with multiple source elements when array is provided', async () => {
        ...
        HTMLElement.prototype.querySelectorAll = jest.fn().mockImplementation(selector => {
          if (selector === 'source') {
            return [
              { getAttribute: (attr: string) => (attr === 'type' ? 'video/mp4' : null) },
              { getAttribute: (attr: string) => (attr === 'type' ? 'video/webm' : null) },
            ]
          }
          if (selector === 'track') {
            return [{ getAttribute: (attr: string) => (attr === 'srclang' ? 'en' : null) }]
        ...
        const page = await newSpecPage({
          components: [WppVideoPlayer],
          template: () => (
            <wpp-video-player src={[videoSrc, videoSrc]} type={['video/mp4', 'video/webm']}></wpp-video-player>
          ),
        })

        const sourceElements = page.root?.shadowRoot?.querySelectorAll('source') as NodeListOf<HTMLSourceElement>
      ...
      it('should create track elements when captions are provided', async () => {
        const captionData: CaptionValue = {
          label: 'English',
          kind: 'subtitles',
          src: 'en-captions.vtt',
          srclang: 'en',
        ...
        const page = await newSpecPage({
          components: [WppVideoPlayer],
          template: () => <wpp-video-player src={videoSrc} caption={captionData}></wpp-video-player>,
        })

        const trackElements = page.root?.shadowRoot?.querySelectorAll('track') as NodeListOf<HTMLTrackElement>
      ...
      it('should render without volume button and volume need to be set to 0', async () => {
        const page = await newSpecPage({
          components: [WppVideoPlayer],
          template: () => (
            <wpp-video-player src={videoSrc} controlPanelConfig={{ showVolumeButton: false }}></wpp-video-player>
          ),
        })
    ...
    describe('rendering methods', () => {
      ...
      it('should render captions component when captions are available', async () => {
        ...
        page.rootInstance.activeCues = [{ text: 'Sample caption text' } as any]
    ...
    describe('methods and functionality', () => {
      it('should get metadata after component got created', async () => {
        ...
        expect(page.rootInstance.videoPlayerRef.style.setProperty).toHaveBeenCalledWith(
          '--aspect-ratio',
          '1.7777777777777777',
        )
      })
      ...
      it('should show volume slider when volumeSliderShow is called', async () => {
        ...
        page.rootInstance.volumeBarContainerRef = mockBarContainer as any
        page.rootInstance.volumeButtonRef = mockButton as any
      ...
      it('should hide volume slider when volumeSliderHide is called', async () => {
        let timeoutCallback: TimerHandler = jest.fn()
        let timeoutDelay = 0
        ...
        jest.spyOn(global, 'setTimeout').mockImplementation((handler: TimerHandler, timeout?: number) => {
      ...
      it('should clear previous timeout when volumeSliderHide is called multiple times', async () => {
        ...
        // Set a hoverTimeout
        page.rootInstance.hoverTimeout = 123 as any

        // Mock clearTimeout
        const clearTimeoutSpy = jest.spyOn(global, 'clearTimeout').mockImplementation(jest.fn())

        // Mock setTimeout to return a new timeout ID
        jest.spyOn(global, 'setTimeout').mockImplementation(() => 456 as any)
    ...
    describe('utils', () => {
      it('should format time correctly', () => {
        ...
        const splitFormat = formatTime('default', 65, true) as any

        expect(splitFormat.minutes).toBe('01')
        expect(splitFormat.seconds).toBe('05')

        const splitFormatExtended = formatTime('extended', 3665, true) as any
    ...
    describe('watchers', () => {
      it('should update splitCurrentVideoTime when currentVideoTime changes', async () => {
        ...
        page.rootInstance.videoPlayerRef = {} as any
