...
} as Meta<Components.WppSideModal>

const getModalHandlers = () => {
  let modal: HTMLWppSideModalElement | null = null

  document.addEventListener(
    'DOMContentLoaded',
    () => (modal = document.querySelector(transformToVersionedTag('wpp-side-modal'))),
  )
...
type SideModalStoryArgs = Components.WppSideModal & { withActions: boolean }

export const SideModal: StoryObj<SideModalStoryArgs> = (args: SideModalStoryArgs) => {
  const { handleOpenModal, handleCloseModal, handleActionModal } = getModalHandlers()

  return html`
    <div style="height: 1200px">
      <wpp-button-v3-2-0 @click=${handleOpenModal}>Open Side Modal</wpp-button-v3-2-0>
      <wpp-side-modal-v3-2-0
        @wppSideModalClose=${() => {
      ...
      >
        ...
        ${args.withActions
          ? html` <div slot="actions" style="display:flex; justify-content: flex-end;">
              ...
              <wpp-button-v3-2-0 variant="primary" .width=${'86px'} @click=${handleActionModal}>
                Action
              </wpp-button-v3-2-0>
            </div>`
          : null}
...
export const SideModalWithActionsConfig: StoryObj<Components.WppSideModal> = (args: Components.WppSideModal) => {
  const { handleOpenModal, handleCloseModal } = getModalHandlers()

  return html`
    <div style="height: 1200px">
      <wpp-button-v3-2-0 @click=${handleOpenModal}>Open Side Modal with Actions Config</wpp-button-v3-2-0>
      <wpp-side-modal-v3-2-0
        @wppSideModalClose=${() => {
          console.log('Called wppSideModalClose')
          handleCloseModal()
