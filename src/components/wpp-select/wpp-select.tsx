...
export class WppSelect implements BaseComponent, BaseFormControl<SelectValue[] | SelectValue>, InlineMessage {
  ...
  protected _locales: SelectLocaleInterface = LOCALES_DEFAULTS
   ...
   */
  @Prop() readonly consistentSearch: boolean = false
   ...
   * @internal
   */
  @Prop() readonly isTextSelect: boolean = false
   ...
   */
  @Prop() readonly getItemKey: (value: ListValue) => string | number | undefined
   ...
   */
  @Prop() readonly locales: Partial<SelectLocaleInterface> = {}
   ...
   * NOTE: Used only in `WppCombinedSelect`
   */
  @Prop() readonly inputType: InputTypes = 'text'
  ...
  @Watch('locales')
  onUpdateLocales(newLocales: Partial<SelectLocaleInterface>) {
  ...
  private convertValueToKey = (value?: ListValue): number | string | undefined => {
    ...
    if (this.loading) {
      return (
        <div class="loading-container">
          <wpp-spinner />
          <wpp-typography type="s-body">{this._locales.loadingText}</wpp-typography>
        </div>
      )
    }
        ...
        {this.internalList?.map((item: ListItemInterface) => {
          ...
          return (
            <wpp-list-item
              ...
              key={this.convertValueToKey(item.value)}
              {...rest}
  ...
  private setHasBeenInternallyDisabled = (listItem: ListItemInterface) => {
    if (listItem.hasBeenInternallyDisabled) return true

    if (listItem.disabled || listItem.checked) return null
  ...
  private onClickListItemMultiple = (listItemValue: ListValue) => {
    ...
    if (valueItem) {
      this.emittedValue = this.value
        ? [...this.value.filter((item: SelectOption) => !isEqual(item, listItemValue))]
        : []
    } else {
