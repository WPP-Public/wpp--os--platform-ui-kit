...
turndownService.escape = (text: string): string => text.replace(/([\\`{}[\]()#+\-.!])/g, '\\$1')
...
turndownService.addRule('underline', {
  ...
  replacement: (content: string): string => `<u>${content}</u>`,
})
...
turndownService.addRule('customHeading', {
  filter(node: Node): boolean {
    return !!node.nodeName && /^H[1-6]$/.test(node.nodeName) && Boolean((node as HTMLElement).textContent?.trim())
  },
...
turndownService.addRule('fencedCodeBlock', {
  filter: (node: HTMLElement): boolean => node.nodeName === 'PRE',
  replacement: (content: string, node: HTMLElement): string => {
    let language = ''
