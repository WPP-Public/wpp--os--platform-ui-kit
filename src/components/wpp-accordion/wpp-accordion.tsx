...
export class WppAccordion {
  ...
  private titleTagsWrapperButtonRef?: HTMLButtonElement
  ...
  private onClick = (event: Event) => {
    if (this.disabled || (event.target as HTMLElement).getAttribute('slot') === 'actions') {
  ...
  private onKeyDown = (event: KeyboardEvent) => {
    if (this.disabled || this.focusType === FOCUS_TYPE.NONE) return
  ...
  render() {
    ...
    const titleContent = (
      <Fragment>
        {this.hasHeaderSlot ? (
          ...
          <wpp-typography type={this.typographyType()} part="title" role="presentation" class="title-text">
            {this.text}
          </wpp-typography>
        )}
      </Fragment>
    ...
    return (
      ...
      >
        <div class={this.cssSectionClasses()} part="section">
          <button
            ref={ref => (this.titleTagsWrapperButtonRef = ref)}
            class={this.cssTagWrapperClasses()}
            aria-expanded={this.expanded.toString()}
            aria-controls={this.ariaProps?.controls ?? 'expandable-panel'}
            id={this.ariaProps?.labelledby ?? 'expandable-button'}
            onFocus={this.onFocus}
            onClick={this.onClick}
            disabled={this.disabled}
          >
            <div class="title-wrapper" part="title-wrapper" role="none">
              <wpp-icon-chevron role="presentation" part="icon" />
              {this.isTitleOverflowing ? (
                <wpp-tooltip config={{ triggerTarget: this.titleTagsWrapperButtonRef }} text={tooltipText}>
                  {titleContent}
                </wpp-tooltip>
              ) : (
                titleContent
        ...
        <div
          ...
          part="content"
          role="region"
          id={this.ariaProps?.controls ?? 'expandable-panel'}
          aria-labelledby={this.ariaProps?.labelledby ?? 'expandable-button'}
          {...(!this.expanded ? { inert: true, 'aria-hidden': true } : {})}
        >
