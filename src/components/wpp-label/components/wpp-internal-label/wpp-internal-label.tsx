import { Component, h, Prop, State, Element, Host, Watch } from '@stencil/core'
...
export class WppInternalLabel {
  private _locales: LabelLocales = LOCALES_DEFAULTS
   ...
   */
  @Prop() readonly locales: Partial<LabelLocales> = {}
  ...
  @Watch('locales')
  onUpdateLocales(newLocales: Partial<LabelLocales>) {
        ...
        {!!this.description && this.hasIconSlot ? (
          <wpp-tooltip class="tooltip" text={this.description} config={this.tooltipConfig} part="tooltip">
            <WrappedSlot
              ...
              role={this.tooltipConfig.tabIndex === -1 ? 'none' : 'button'}
              tabIndex={this.tooltipConfig.tabIndex ?? 0}
              aria-label={this.tooltipConfig.tabIndex !== -1 ? 'Show info' : undefined}
            />
          </wpp-tooltip>
        ) : (
          <WrappedSlot
