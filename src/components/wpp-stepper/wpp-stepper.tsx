...
export class WppStepper {
  ...
  componentDidLoad() {
    ...
    setTimeout(() => {
      ...
      const stepMainList = this.host.querySelectorAll<HTMLWppStepElement>(
        `:scope > ${transformToVersionedTag('wpp-step')}`,
      )
  ...
  private setStepAttributeAsIntegers = () => {
    ...
    stepList.forEach((step, index) => {
      if (this.stepMainNdx === index + 1) step.classList.add('wpp-last-step')
