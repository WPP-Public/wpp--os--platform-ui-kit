...
export class WppDatepicker implements BaseComponent, InlineMessage {
  ...
  private _locales: LocaleTypes = LOCALES_DEFAULTS
  ...
  @State() isValueExists: boolean = false
   ...
   */
  @Prop() readonly locale: Partial<LocaleTypes> = {}
   ...
   * - `dateLocale` is used to automatically infer date-related properties, like `firstDay`.
   */
  @Prop() readonly locales: Partial<LocaleTypes> = {}
  ...
  updateValue() {
    ...
    if (this.range) {
      ...
      this.isValueExists = Boolean(((this.value as string) ?? '').trim())
    }
  }
  ...
  @Watch('locales')
  onUpdateLocales(newLocales: Partial<LocaleTypes>) {
  ...
  private determineFirstDay(): 0 | 1 | 2 | 3 | 4 | 5 | 6 {
    if (this._locales.dateLocale) {
      ...
      if (mappedFirstDay !== undefined) {
        ...
        console.warn(
          `Unknown dateLocale: "${this._locales.dateLocale}". Defaulting to firstDay: Monday (1). ` +
