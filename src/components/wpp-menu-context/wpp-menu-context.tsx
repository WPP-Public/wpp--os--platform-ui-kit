import { Component, h, Host, Element, State, Prop, Listen, Watch, Event, EventEmitter } from '@stencil/core'
...
export class WppMenuContext {
   ...
   */
  @Event({ bubbles: false, composed: false }) wppBlur: EventEmitter<void>
  ...
  private handleClick(event: Event) {
    ...
    this.isTriggerDisabled = !!(
      (this.triggerElement?.hasAttribute('disabled') && this.triggerElement?.getAttribute('disabled') !== 'false') ||
  ...
  componentWillLoad() {
    this.isNestedContext =
      (this.host?.children[0] as HTMLElement)?.tagName === transformToVersionedTag(CONTEXT_ITEM_TAG).toUpperCase()
  }
  ...
  componentDidLoad() {
    ...
    if (this.triggerRef) {
      this.triggerElement = this.triggerRef?.querySelector('[slot="trigger-element"]') as HTMLElement
  ...
  private createTippyInstance = () => {
    ...
    this.tippyInstance = menuListConfig({
      ...
      onHidden: () => {
        if (document.activeElement === this.triggerElement) return
  ...
  private onFocus = (event: FocusEvent) => {
    if (!this.isInComponent) this.wppFocus.emit(event)
  ...
  private onFocusout = (event: FocusEvent) => {
    if (
      this.host.contains(event.relatedTarget as Node) ||
      this.tippyInstance.popper.contains(event.relatedTarget as Node)
    )
      return
  ...
  render() {
    ...
    return (
      <Host class={this.menuCssClasses()} exportparts="trigger, list-wrapper, list, inner" onFocusout={this.onFocusout}>
        <div ref={this.getTriggerRef} class={this.triggerWrapperCssClasses()}>
