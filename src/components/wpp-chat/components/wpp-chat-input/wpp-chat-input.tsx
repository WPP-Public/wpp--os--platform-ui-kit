...
export class WppChatInput {
   ...
   */
  @Prop() readonly zIndex: number = Z_INDEX.CHAT
   ...
   */
  private handleFilePaste = async (event: ClipboardEvent, items: DataTransferItemList): Promise<boolean> => {
    ...
    if (files.length > 0) {
      ...
      await this.handleFileLoad(files as any)
  ...
  private adjustTextareaHeight = (
    reset: boolean = false,
    value?: string,
    waitForInputArea: boolean = false,
  ): Promise<void> =>
    new Promise(resolve => {
  ...
  private isMaximumFilesSet = (): boolean => this.mergedFileUploadConfig.maxFiles > 0
