...
const WppLegacyAutocomplete$1 = /*@__PURE__*/ proxyCustomElement(class WppLegacyAutocomplete extends HTMLElement {
  constructor() {
    ...
    this.renderInputPlaceholder = () => {
      if (!this.multiple && this.value.length && !this.isFocused) {
        return (h("wpp-typography-v4-2-0", { "data-testid": "wpp-autocomplete-input-placeholder", type: "s-body", class: "input-placeholder" }, this.getOptionLabel(this.value[0])));
      }
      ...
      if (this.type === 'regular') {
        ...
        return (h("wpp-typography-v4-2-0", { "data-testid": "wpp-autocomplete-input-placeholder", type: "s-body", class: "input-placeholder" }, placeholder));
      }
      if (this.type === 'extended' && this.value.length && !this.isFocused) {
        return (h("wpp-typography-v4-2-0", { "data-testid": "wpp-autocomplete-input-placeholder", type: "s-body", class: "input-placeholder" }, this._locales.selected(this.value.length)));
      }
    };
    this.countHiddenElements = () => {
    ...
    this.renderDropdownContent = () => {
      ...
      if (isLoading) {
        return (h("div", { class: "loading-wrapper" }, h("wpp-spinner-v4-2-0", { slot: "left" }), h("wpp-typography-v4-2-0", { type: "s-body", slot: "label" }, this._locales.loading)));
      }
      if (isEmptyStringEntered && this.componentSuggestions?.length > 0) {
        return (h(Fragment, null, h("wpp-typography-v4-2-0", { type: "s-strong", class: "suggestions-heading" }, this.suggestionsTitle), this.componentSuggestions?.map((suggestion, index) => {
          const { slots, checked, label, ...restProps } = suggestion;
          const isChecked = checked || this.isItemSelected(suggestion);
          return (h("wpp-list-item-v4-2-0", { key: this.getOptionId(suggestion), selectable: true, checked: isChecked, value: suggestion, onWppChangeListItem: this.handleSuggestionClick, class: { 'suggestion-item': true, 'last-item': index === this.componentSuggestions?.length - 1 }, ...restProps }, label && h("span", { slot: "label" }, this.getOptionLabel(suggestion)), slots && this.renderSlotsListItem(slots, Boolean(label)).map(slotNode => slotNode)));
        })));
      }
      if (this.isEmptyOptions) {
        return (h(Fragment, null, h("wpp-list-item-v4-2-0", { class: "nothing-found-wrapper" }, h("wpp-typography-v4-2-0", { type: "s-body", class: "nothing-found", slot: "label" }, this._locales.nothingFound))));
      }
      return (h(Fragment, null, !this.searchValue ? (!this.withPills ? (h("div", { class: this.selectedValuesCssClasses(), part: "selected-values" }, h("slot", { name: "selected-values" }))) : null) : (h("slot", null)), h("div", null, this.isInfiniteLoading && (h("div", { class: "infinite-loader" }, h("wpp-spinner-v4-2-0", null))))));
    };
    ...
    this.renderPillComponent = (option, isTooltip, isTransparentTooltip = false, isTransparentPill = false) => {
      ...
      if (isTooltip) {
        return (h("wpp-tooltip-v4-2-0", { class: {
            'in-dropdown': true,
            transparent: isTransparentTooltip,
          }, text: labelText, part: "tooltip", config: { ...this.pillTooltipConfig } }, h("wpp-pill-v4-2-0", { class: { transparent: isTransparentPill }, label: labelText, type: "display", removable: true, onWppClose: event => this.handleSuggestionClick({ ...event, ...{ detail: { value: option } } }) })));
      }
      else {
        return (h("wpp-pill-v4-2-0", { class: { transparent: isTransparentPill }, label: labelText, type: "display", removable: true, onWppClose: event => this.handleSuggestionClick({ ...event, ...{ detail: { value: option } } }) }));
      }
    };
    /**
    ...
    this.showMoreLessRender = (label) => (h("wpp-action-button-v4-2-0", { "data-testid": "wpp-autocomplete-show-btn", class: "nowrap", variant: "secondary", onClick: this.handleShowMoreLessClick }, label));
    this.handleShowMoreLessClick = () => {
  ...
  renderSelectedOptions() {
    ...
    })), h("div", { class: "show-more-action" }, this.showMoreLessRender(`+${this.activePillsTruncationState.filter(x => x).length} ${this._locales.showMore}`)))), !this.isShowMore && h("div", { class: "show-less-action" }, this.showMoreLessRender(this._locales.showLess)), isNeedDivider && h("wpp-divider-v4-2-0", { class: "nothing-found-divider" })));
  }
  render() {
    const style = { '--custom-dropdown-width': this.getDropdownWidth() };
    return (h(Host, { style: this.hostStyle(), class: this.hostCssClasses(), onFocus: this.handleFocus, onBlur: this.handleBlur, onMouseDown: this.handleMouseDown, onKeyUp: this.handleKeyUp, "aria-disabled": this.disabled, "aria-required": this.required, exportparts: "input, dropdown, options, selected-values" }, h("div", { class: this.autocompleteWrapperCssClasses(), onMouseDown: this.handleTriggerContainerMouseDown }, this.labelConfig?.text && (h("wpp-label-v4-2-0", { class: this.labelCssClasses(), htmlFor: this.name, disabled: this.disabled, optional: !this.required, config: this.labelConfig, tooltipConfig: this.labelTooltipConfig })), h("div", { ref: triggerEl => (this.triggerEl = triggerEl), class: this.triggerCssClasses(), onClick: this.handleTriggerClick }, h("div", { ref: valuesEl => (this.valuesContainerEl = valuesEl), class: "values" }, this.renderInputPlaceholder(), this.hiddenSelectedOptionsNumber > 0 && !this.isDropdownShown && (h("div", { class: "hidden-count", style: { '--hidden-number': this.getNearestPowForRowsNumber() + '' } }, h("wpp-typography-v4-2-0", { type: "s-body" }, ", + ", this.hiddenSelectedOptionsNumber))), h("input", { part: "input", ref: inputEl => (this.inputEl = inputEl), class: this.inputCssClasses(), id: this.name, name: this.name, type: "text", value: this.getInputValue(), disabled: this.disabled, placeholder: this.placeholder, required: this.required, autocomplete: "off", onInput: this.handleInput, tabIndex: this.disabled ? -1 : 0, title: "" })), h("div", { class: "trigger-actions" }, this.hasClearButton() && h("wpp-icon-cross-v4-2-0", { onClick: this.handleClearClick }))), !!this.message && (h("wpp-inline-message-v4-2-0", { class: "inline-message", showTooltipFrom: this.maxMessageLength, message: this.message, type: this.messageType }))), h("div", { class: "dropdown", part: "dropdown", ref: dropdownEl => (this.dropdownEl = dropdownEl), style: style }, h("div", { ref: optionsListEl => (this.optionsListEl = optionsListEl), part: "options", class: this.dropdownListCssClasses(), onScroll: this.handleOptionsScroll }, this.renderSelectedOptions(), this.renderDropdownContent(), this.showCreateNewElement &&
      this.searchValue !== '' &&
      ((this.displayBtnWhenListEmpty && this.isEmptyOptions) || !this.displayBtnWhenListEmpty) && (h("div", { class: "actions" }, h("wpp-divider-v4-2-0", { class: "nothing-found-divider" }), h("div", { class: "actions-container" }, h("wpp-list-item-v4-2-0", { onClick: this.handleCreateNewOptionClick }, h("wpp-typography-v4-2-0", { type: "s-strong", class: "create-new-option", slot: "label" }, this._locales.createNewElement))))))), this.type === 'extended' && this.multiple ? h("slot", { name: "selected-values" }) : null));
  }
...
}, [1, "wpp-legacy-autocomplete", "wpp-legacy-autocomplete-v4-2-0", {
    "name": [1],
    "loading": [516],
    "disabled": [516],
...
function defineCustomElement$1() {
  ...
  const components = ["wpp-legacy-autocomplete-v4-2-0", "wpp-action-button-v4-2-0", "wpp-checkbox-v4-2-0", "wpp-divider-v4-2-0", "wpp-icon-chevron-v4-2-0", "wpp-icon-cross-v4-2-0", "wpp-icon-dash-v4-2-0", "wpp-icon-drag-v4-2-0", "wpp-icon-error-v4-2-0", "wpp-icon-info-message-v4-2-0", "wpp-icon-success-v4-2-0", "wpp-icon-tick-v4-2-0", "wpp-icon-warning-v4-2-0", "wpp-inline-message-v4-2-0", "wpp-internal-label-v4-2-0", "wpp-internal-tooltip-v4-2-0", "wpp-label-v4-2-0", "wpp-list-item-v4-2-0", "wpp-pill-v4-2-0", "wpp-spinner-v4-2-0", "wpp-tooltip-v4-2-0", "wpp-typography-v4-2-0"];
  components.forEach(tagName => { switch (tagName) {
    case "wpp-legacy-autocomplete-v4-2-0":
