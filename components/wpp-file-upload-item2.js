...
const returnIconFromExtension = (fileExtension, thumbnailUrl) => {
  ...
  switch (fileExtension) {
    // Text
    case '.txt':
      return h("wpp-icon-document-v3-3-0", null);
    // Compressed
    case '.zip':
    case '.rar':
    case '.7z':
      return h("wpp-icon-file-zip-v3-3-0", null);
    // Image
    case '.png':
    case '.jpg':
    case '.jpeg':
    case '.svg':
    case '.gif':
      return h("wpp-icon-image-v3-3-0", null);
    // Video
    case '.mp4':
    case '.mov':
    case '.avi':
    ...
    case '.flv':
    case '.webm':
      return h("wpp-icon-video-clip-v3-3-0", null);
    // Audio
    case '.mp3':
    case '.wav':
    case '.ogg':
    ...
    case '.m4a':
    case '.aac':
      return h("wpp-icon-music-v3-3-0", null);
    // Data
    case '.csv':
    case '.json':
    case '.xml':
    ...
    case '.sqlite':
    case '.dat':
      return h("wpp-icon-database-v3-3-0", null);
    // Presentation
    case '.pptx':
    case '.key':
    case '.odp':
    ...
    case '.sldx':
    case '.ppt':
      return h("wpp-icon-pitch-v3-3-0", null);
    // Spreadsheet
    case '.xlsx':
    case '.xls':
    case '.ods':
    case '.numbers':
    case '.tsv':
      return h("wpp-icon-spreadsheet-v3-3-0", null);
    default:
      return h("wpp-icon-file-v3-3-0", null);
  }
};
...
const WppFileUploadItem = /*@__PURE__*/ proxyCustomElement(class WppFileUploadItem extends HTMLElement {
  constructor() {
    ...
    this.setCurrentIcon = () => {
      if (this.isFileLoading())
        return h("wpp-spinner-v3-3-0", null);
    ...
    this.setCurrentError = () => {
      if (this.isFileWithError()) {
        ...
        return (h("div", { class: "error-wrapper" }, h("wpp-inline-message-v3-3-0", { class: "inline-message-error", message: currentError, type: "error", showTooltipFrom: 140, tooltipConfig: { popperOptions: { strategy: 'fixed' } } }), this.file.deletable !== false && (h("wpp-icon-cross-v3-3-0", { class: this.crossIconClasses(), part: "cross-icon", role: "button", tabindex: this.parentDisabled || this.file.disabled ? -1 : 0, "aria-disabled": this.parentDisabled || this.file.disabled ? 'true' : undefined, "aria-label": `Remove file ${this.file.name}`, onClick: this.handleCloseClick, onKeyDown: this.handleDeleteKeyDown, onKeyUp: this.handleDeleteKeyUp, onBlur: this.handleDeleteBlur }))));
      }
      return null;
    };
    this.getEventData = () => ({
  ...
  render() {
    return (h(Host, { class: this.hostCssClasses(), exportparts: "file-item, wrapper, content, file-name, tooltip, loading, percentage, cross-icon", onClick: this.handleClick, role: "listitem" }, h("div", { class: this.itemCssClasses(), part: "file-item" }, this.setCurrentError(), h("div", { class: "content-wrapper", part: "wrapper" }, h("div", { class: this.blockCssClasses(), part: "content" }, h("div", { class: "icon-wrapper" }, this.setCurrentIcon()), h("wpp-tooltip-v3-3-0", { class: this.maxLabelLength ? 'computed' : '', ref: ref => (this.tooltipRef = ref), text: this.file.name, config: {
        popperOptions: { strategy: 'fixed' },
        onShow: () => {
          if (!this.isTruncated || (this.maxLabelLength && !(this.file?.name?.length > this.maxLabelLength)))
            return false;
        },
      }, part: "tooltip" }, h("wpp-typography-v3-3-0", { ref: ref => (this.fileNameRef = ref), class: this.fileNameCssClasses(), type: "s-body", part: "file-name", title: this.file.name }, this.maxLabelLength ? truncate(this.file.name, this.maxLabelLength, true) : this.file?.name)), !this.isFileWithError() && (h("span", { ref: ref => (this.loadingRef = ref), class: "loading", part: "loading" }, this.isFileLoading()
      ? `${this.loaded}/${this.total} ${this.measurementUnit}`
      : `${this.total} ${this.measurementUnit}`))), h("div", { class: "controls-wrapper", part: "controls" }, this.isFileLoading() && (h("span", { class: "percentage", part: "percentage" }, this.percentage, "%")), this.file.deletable !== false && !this.isFileWithError() && (h("wpp-icon-cross-v3-3-0", { class: this.crossIconClasses(), part: "cross-icon", role: "button", tabindex: this.parentDisabled || this.file.disabled ? -1 : 0, "aria-disabled": this.parentDisabled || this.file.disabled ? 'true' : undefined, "aria-label": `Remove file ${this.file.name}`, onClick: this.handleCloseClick, onKeyDown: this.handleDeleteKeyDown, onBlur: this.handleDeleteBlur, onKeyUp: this.handleDeleteKeyUp }))))), h("wpp-typography-v3-3-0", { ref: ref => (this.measureRef = ref), type: "s-body", class: "measure", "aria-hidden": "true", role: "presentation" })));
  }
...
}, [1, "wpp-file-upload-item", "wpp-file-upload-item-v3-3-0", {
...
function defineCustomElement() {
  ...
  const components = ["wpp-file-upload-item-v3-3-0", "wpp-action-button-v3-3-0", "wpp-icon-cross-v3-3-0", "wpp-icon-database-v3-3-0", "wpp-icon-document-v3-3-0", "wpp-icon-error-v3-3-0", "wpp-icon-file-v3-3-0", "wpp-icon-file-zip-v3-3-0", "wpp-icon-image-v3-3-0", "wpp-icon-info-message-v3-3-0", "wpp-icon-music-v3-3-0", "wpp-icon-pitch-v3-3-0", "wpp-icon-spreadsheet-v3-3-0", "wpp-icon-success-v3-3-0", "wpp-icon-video-clip-v3-3-0", "wpp-icon-warning-v3-3-0", "wpp-inline-message-v3-3-0", "wpp-internal-tooltip-v3-3-0", "wpp-spinner-v3-3-0", "wpp-tooltip-v3-3-0", "wpp-typography-v3-3-0"];
