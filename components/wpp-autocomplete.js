...
import { w as getHighestContainerInDOM, d as debounce, s as selectDropdownWidth, y as mergeLocales, b as isEventTargetContained } from './utils.js';
import { Z as Z_INDEX } from './consts.js';
import { i as isSelected, L as LIB_COMPONENTS_PREFIX, r as renderSlotsInListItem, s as selectedOptionsByOrder } from './utils2.js';
...
function renderDropdownPillsComponent() {
  ...
  const showMoreLessRender = (label) => (h("wpp-action-button-v4-3-0", { "data-testid": "wpp-autocomplete-show-btn", class: "nowrap", variant: "secondary", onClick: this.handleShowMoreLessClick }, label));
  const renderPillComponent = (option, ndx, isTransparentPill = false) => (h("wpp-pill-v4-3-0", { ref: ref => {
      if (ref)
  ...
  return (h(Fragment, null,
    ...
    isNeedDivider && h("wpp-divider-v4-3-0", { class: "nothing-found-divider" })));
}
...
function renderPlaceholderTextComponent() {
  ...
  return (h(Fragment, null,
    h("wpp-typography-v4-3-0", { ref: ref => (this.inputPlaceholderRef = ref), type: "s-body", class: inputPlaceholderCssClasses() }, this.placeholderText),
    h("wpp-typography-v4-3-0", { ref: ref => (this.hiddenInputPlaceholderRef = ref), role: "presentation", type: "s-body", class: "hidden-input-placeholder" }),
    this.value.length > 1 && (h("wpp-typography-v4-3-0", { class: hiddenCountCssClasses(), type: "s-body" },
      "+ ",
  ...
  const renderListOptions = () => {
    ...
    return (h(Fragment, null,
      this.internalList.map((item, ndx) => {
        ...
        return (h("wpp-list-item-v4-3-0", { ref: el => (this.listItemsRefs[ndx] = el), onWppChangeListItem: this.handleClickListItem, key: convertValueToKey(item.value), isDarkTheme: this.isDarkTheme, ...rest, id: item.id !== undefined ? `${LIB_COMPONENTS_PREFIX}list-item-${item.id}` : undefined, role: "option" },
      ...
      this.isInfiniteLoading && (h("div", { class: "wpp-dropdown-infinite-loader" },
        h("wpp-spinner-v4-3-0", null)))));
  };
  const renderSuggestionOptions = () => {
    this.suggestionsItemsRefs = [];
    return (h(Fragment, null,
      h("wpp-typography-v4-3-0", { role: "presentation", type: "s-strong", class: "suggestions-heading" }, this._locales.suggestionTitle),
      this.componentSuggestions?.map((suggestion, ndx) => {
        ...
        return (h("wpp-list-item-v4-3-0", { ref: el => (this.suggestionsItemsRefs[ndx] = el), onWppChangeListItem: this.handleClickListItem, key: convertValueToKey(suggestion.value), ...restProps, id: suggestion.id !== undefined ? `${LIB_COMPONENTS_PREFIX}list-item-${suggestion.id}` : undefined, checked: isChecked, isDarkTheme: this.isDarkTheme, class: { 'suggestion-item': true, 'last-item': ndx === this.componentSuggestions.length - 1 }, role: "option" },
          h("span", { slot: "label" }, label),
          slots && renderSlotsInListItem(slots, Boolean(label)).map((slotNode) => slotNode)));
      })));
  };
  if (this.loading) {
    return (h("wpp-list-item-v4-3-0", { isDarkTheme: this.isDarkTheme, role: "status", "non-interactive": true },
      h("wpp-spinner-v4-3-0", { slot: "left" }),
      h("span", { slot: "label" }, this._locales.loading)));
  }
  if (!this.visibleOptionsLength && this.searchText.length > 0) {
    return (h("wpp-list-item-v4-3-0", { isDarkTheme: this.isDarkTheme, labelTypography: { color: 'var(--wpp-grey-color-700)', type: 's-body' }, nonInteractive: true },
      h("wpp-typography-v4-3-0", { type: "s-body", class: "nothing-found", slot: "label" }, this._locales.nothingFound)));
  }
...
function renderExtendedSelectedValuesComponent() {
  return (h("div", { class: "selected-values" }, this.extendedSelectedValues?.map(el => (h("wpp-pill-v4-3-0", { label: el.label, type: "display", onWppClose: event => this.handleClickListItem({
      ...event,
...
const WppAutocomplete$1 = /*@__PURE__*/ proxyCustomElement(class WppAutocomplete extends HTMLElement {
  constructor() {
    ...
    this.themeSubscription = themeSubscriptionController(() => this.dropdownRef, (theme) => {
...
}, [1, "wpp-autocomplete", "wpp-autocomplete-v4-3-0", {
    "labelConfig": [16],
    "name": [1],
...
function defineCustomElement$1() {
  ...
  const components = ["wpp-autocomplete-v4-3-0", "wpp-action-button-v4-3-0", "wpp-checkbox-v4-3-0", "wpp-divider-v4-3-0", "wpp-icon-chevron-v4-3-0", "wpp-icon-cross-v4-3-0", "wpp-icon-dash-v4-3-0", "wpp-icon-drag-v4-3-0", "wpp-icon-error-v4-3-0", "wpp-icon-info-message-v4-3-0", "wpp-icon-search-v4-3-0", "wpp-icon-success-v4-3-0", "wpp-icon-tick-v4-3-0", "wpp-icon-warning-v4-3-0", "wpp-inline-message-v4-3-0", "wpp-input-v4-3-0", "wpp-internal-label-v4-3-0", "wpp-internal-tooltip-v4-3-0", "wpp-label-v4-3-0", "wpp-list-item-v4-3-0", "wpp-pill-v4-3-0", "wpp-spinner-v4-3-0", "wpp-tooltip-v4-3-0", "wpp-typography-v4-3-0"];
  components.forEach(tagName => { switch (tagName) {
    case "wpp-autocomplete-v4-3-0":
