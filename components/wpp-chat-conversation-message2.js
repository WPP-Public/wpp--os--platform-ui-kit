...
const renderImageNode = (role, data, className) => {
  ...
  return data.linkHref ? (h("a", { href: data.linkHref, target: "_blank", rel: "noopener noreferrer", class: "chat-image-link" }, img)) : role === 'assistant' ? (h(Fragment, null,
    h("wpp-action-button-v4-0-0", { class: "image-download-button", variant: "secondary", onClick: () => handleDownload(data.href, data.alt) },
      h("wpp-icon-download-v4-0-0", { slot: "icon-start", "aria-hidden": "true" })),
    img)) : (img);
};
  ...
  switch (token.type) {
    ...
    case 'code':
      return (h("pre", { class: "code-block" },
        h("div", { class: "code-lang" },
          ...
          h("wpp-icon-copy-v4-0-0", { class: "code-block-copy-icon", size: "s", color: "var(--wpp-grey-color-600)", onClick: () => {
    ...
    case 'list_item': {
      ...
      return (h("li", { class: `chat-list-item ${token.task ? `chat-list-item-task ${token.checked ? 'chat-list-item-task-checked' : ''}` : ''}` },
        token.task && h("wpp-icon-tick-v4-0-0", null),
        inlineContent.length > 0 && (h("wpp-typography-v4-0-0", { type: "s-body" }, inlineContent.map(t => renderToken(t)))),
        ...
        h("table", { class: "chat-table" },
          h("thead", null,
            h("tr", null, token.header.map((cell, i) => (h("th", { key: i },
              h("wpp-typography-v4-0-0", { type: "s-strong" }, cell.tokens?.map((t) => renderToken(t)) ?? cell.text)))))),
          h("tbody", null, token.rows.map((row, r) => (h("tr", { key: r }, row.map((cell, c) => (h("td", { key: c },
            h("wpp-typography-v4-0-0", { type: "s-body" }, cell.tokens?.map((t) => renderToken(t)) ?? cell.text)))))))))));
    }
    case 'link':
      if (token.title) {
        return (h("wpp-tooltip-v4-0-0", { text: token.title },
...
const WppChatConversationMessage = /*@__PURE__*/ proxyCustomElement(class WppChatConversationMessage extends HTMLElement {
  ...
  renderComplete() {
    ...
    if (!Array.isArray(tokens)) {
      return (h("wpp-typography-v4-0-0", { type: "s-body", tag: "p" }, purify.sanitize(this.finalContent)));
    }
...
}, [1, "wpp-chat-conversation-message", "wpp-chat-conversation-message-v4-0-0", {
    "role": [1],
    "content": [1],
    "status": [1],
