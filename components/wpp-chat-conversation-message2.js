...
const renderImageNode = (role, data, className) => {
  const img = (h(Fragment, null,
    ...
    h("wpp-icon-image-v4-1-0", { role: "presentation", "aria-hidden": "true" })));
  return data.linkHref ? (h("a", { href: data.linkHref, target: "_blank", rel: "noopener noreferrer", class: "chat-image-link" }, img)) : role === 'assistant' ? (h(Fragment, null,
    h("wpp-action-button-v4-1-0", { class: "image-download-button", variant: "secondary", onClick: () => handleDownload(data.href, data.alt) },
      h("wpp-icon-download-v4-1-0", { slot: "icon-start", "aria-hidden": "true" })),
    img)) : (img);
};
const renderToken = (token, role = 'user') => {
  switch (token.type) {
    ...
    case 'heading': {
      ...
      return (h("wpp-typography-v4-1-0", { type: type, tag: tag }, token.tokens ? token.tokens.map(t => renderToken(t)) : token.text));
    }
    case 'del':
      return h("del", null, token.tokens?.map(t => renderToken(t)));
    case 'paragraph':
      return (h("wpp-typography-v4-1-0", { type: "s-body", tag: "p" }, token.tokens ? token.tokens.map(t => renderToken(t)) : token.text));
    case 'blockquote':
      return h("blockquote", { class: "chat-quote" }, token.tokens?.map(t => renderToken(t)));
    case 'strong':
      return (h("wpp-typography-v4-1-0", { type: "s-strong", tag: "span" }, token.tokens?.map(t => renderToken(t))));
    case 'em':
    ...
    case 'code':
      return (h("pre", { class: "code-block" },
        h("div", { class: "code-lang" },
          ...
          h("wpp-icon-copy-v4-1-0", { class: "code-block-copy-icon", size: "s", color: "var(--wpp-grey-color-600)", onClick: () => {
    ...
    case 'list_item': {
      ...
      return (h("li", { class: `chat-list-item ${token.task ? `chat-list-item-task ${token.checked ? 'chat-list-item-task-checked' : ''}` : ''}` },
        token.task && h("wpp-icon-tick-v4-1-0", null),
        inlineContent.length > 0 && (h("wpp-typography-v4-1-0", { type: "s-body" }, inlineContent.map(t => renderToken(t)))),
        ...
        h("table", { class: "chat-table" },
          h("thead", null,
            h("tr", null, token.header.map((cell, i) => (h("th", { key: i },
              h("wpp-typography-v4-1-0", { type: "s-strong" }, cell.tokens?.map((t) => renderToken(t)) ?? cell.text)))))),
          h("tbody", null, token.rows.map((row, r) => (h("tr", { key: r }, row.map((cell, c) => (h("td", { key: c },
            h("wpp-typography-v4-1-0", { type: "s-body" }, cell.tokens?.map((t) => renderToken(t)) ?? cell.text)))))))))));
    }
    case 'link':
      if (token.title) {
        return (h("wpp-tooltip-v4-1-0", { text: token.title },
    ...
    case 'hr':
      return h("wpp-divider-v4-1-0", null);
    case 'text':
      return token.tokens
    ...
    case 'tag':
      ...
      return null;
    default:
      return 'text' in token ? h("wpp-typography-v4-1-0", { type: "s-body" }, token.text) : null;
  }
};
...
const WppChatConversationMessage = /*@__PURE__*/ proxyCustomElement(class WppChatConversationMessage extends HTMLElement {
  ...
  renderComplete() {
    ...
    if (!Array.isArray(tokens)) {
      return (h("wpp-typography-v4-1-0", { type: "s-body", tag: "p" }, purify.sanitize(this.finalContent)));
    }
...
}, [1, "wpp-chat-conversation-message", "wpp-chat-conversation-message-v4-1-0", {
    "role": [1],
    "content": [1],
    "status": [1],
...
function defineCustomElement() {
  ...
  const components = ["wpp-chat-conversation-message-v4-1-0", "wpp-action-button-v4-1-0", "wpp-avatar-v4-1-0", "wpp-checkbox-v4-1-0", "wpp-divider-v4-1-0", "wpp-file-upload-item-v4-1-0", "wpp-icon-chevron-v4-1-0", "wpp-icon-copy-v4-1-0", "wpp-icon-cross-v4-1-0", "wpp-icon-dash-v4-1-0", "wpp-icon-database-v4-1-0", "wpp-icon-document-v4-1-0", "wpp-icon-download-v4-1-0", "wpp-icon-error-v4-1-0", "wpp-icon-file-v4-1-0", "wpp-icon-file-zip-v4-1-0", "wpp-icon-image-v4-1-0", "wpp-icon-info-message-v4-1-0", "wpp-icon-more-v4-1-0", "wpp-icon-music-v4-1-0", "wpp-icon-pitch-v4-1-0", "wpp-icon-spreadsheet-v4-1-0", "wpp-icon-success-v4-1-0", "wpp-icon-tick-v4-1-0", "wpp-icon-video-clip-v4-1-0", "wpp-icon-warning-v4-1-0", "wpp-inline-message-v4-1-0", "wpp-internal-label-v4-1-0", "wpp-internal-tooltip-v4-1-0", "wpp-label-v4-1-0", "wpp-list-item-v4-1-0", "wpp-menu-context-v4-1-0", "wpp-spinner-v4-1-0", "wpp-tooltip-v4-1-0", "wpp-typography-v4-1-0"];
  components.forEach(tagName => { switch (tagName) {
    case "wpp-chat-conversation-message-v4-1-0":
