...
const getEmptyStates = () =>
  imageList.groups[0].images.map(
    image => html`
      <div style=${styleMap(itemWrapper)}>
        ${unsafeHTML(
          `<${transformToVersionedTag(`wpp-empty-${image.name}`)}></${transformToVersionedTag(
            `wpp-empty-${image.name}`,
          )}>`,
        )}
        <wpp-typography-v3-1-1 type="xs-body" style=${styleMap(labelStyle)}>${image.label}</wpp-typography-v3-1-1>
      </div>
    `,
  )
...
export const EmptyStates = () => html`
  <div style=${styleMap(pageStyle)}>
    <div style=${styleMap(pageWrapper)}>
      <wpp-typography-v3-1-1 type="3xl-heading" tag="h3" style=${styleMap(headerStyle)}>
        Empty States
