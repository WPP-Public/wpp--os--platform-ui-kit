...
import { StepChangeEventDetail } from './types'
...
const getVerticalStepperHandlers = (type: 'normal' | 'with-width') => {
  let stepper: HTMLWppStepperElement | null = null
  let pageList: HTMLElement | null = null
  ...
  const handleNextClick = () => {
    if (type === 'normal') {
      if (stepperState.currentStep >= LAST_PAGE_COUNT) return
    } else {
      if (stepperState.currentStep >= LAST_PAGE_COUNT_WITH_WIDTH) return
    }

    if (!stepper?.activeStep || !pageList) return
  ...
  const handlePreviousClick = () => {
    if (stepperState.currentStep === FIRST_PAGE_COUNT) return
    if (!stepper?.activeStep || !pageList) return
  ...
  const handleStepClick = (event: CustomEvent<StepChangeEventDetail>) => {
    ...
    if (!clickedIndex || !pageList || !stepper?.activeStep) return
  ...
  const handleCheckboxClick = () => {
    ...
    const stepError = document.querySelector('#step') as HTMLWppStepElement
...
const getVerticalDecimalStepperHandlers = () => {
  ...
  const stepsMap: { [key: number]: { next?: number; prev?: number; pageIndex: number } } = {
  ...
  const getPageOffset = (step: number) => {
  ...
  const handleNextClick = () => {
    if (stepperState.currentStep >= DECIMAL_LAST_PAGE_COUNT) return
    if (!stepper?.activeStep || !pageList) return

    if (DECIMAL_REQUIRED_STEP_NUMBER === stepperState.currentStep && !stepperState.checked) {
      const stepError = document.querySelector('#step3_2') as HTMLWppStepElement
  ...
  const handlePreviousClick = () => {
    if (stepperState.currentStep === DECIMAL_FIRST_PAGE_COUNT) return
    if (!stepper?.activeStep || !pageList) return
  ...
  const handleStepClick = (event: CustomEvent<StepChangeEventDetail>) => {
    ...
    if (!stepper?.activeStep || !pageList || !clickedIndex) return
...
const getHorizontalStepperHandlers = () => {
  ...
  const handleHorizontalNextClick = () => {
    if (stepperState.currentStep >= HORIZONTAL_LAST_PAGE_COUNT) return
    if (!stepper?.activeStep || !pageList) return
  ...
  const handleHorizontalPreviousClick = () => {
    if (stepperState.currentStep === HORIZONTAL_FIRST_PAGE_COUNT) return
    if (!stepper?.activeStep || !pageList) return
  ...
  const handleHorizontalStepClick = (event: CustomEvent<StepChangeEventDetail>) => {
    const clickedIndex = event.detail.index
...
export const VerticalDecimalStepper: Story<Props> = (args: Props) => {
  const { handleNextClick, handlePreviousClick, handleStepClick, handleCheckboxClick, stepperState } =
    getVerticalDecimalStepperHandlers()
  ...
  return html`
    <div>
      <div style="display: flex;">
        <div style="width: 200px; text-align: center;">
          ...
          >
            ...
            <wpp-step-v3-1-1>
              ...
              <wpp-step-v3-1-1 .substep=${true}>
                <p slot="label">Sub step 2.1</p>
              </wpp-step-v3-1-1>
              <wpp-step-v3-1-1 id="step" .substep=${true}>
        ...
        <div style=${styleMap(pageWrapper)}>
          <div id="page_list" style=${styleMap(pageListStyle)}>
            ...
            <div style=${styleMap(pageStyle)}>
              <h3>Sub page 3.2</h3>
              <wpp-checkbox-v3-1-1 style="margin-left: 16px;" @click=${handleCheckboxClick} />
            </div>
            <div style=${styleMap(pageStyle)}>
              <h3>Sub page 3.3</h3>
            </div>
      ...
      <div style=${styleMap(buttonWrapper)}>
        <wpp-button-v3-1-1 .variant=${'secondary'} @click=${handlePreviousClick}>Previous Step</wpp-button-v3-1-1>
        <wpp-button-v3-1-1 @click=${handleNextClick} style="margin-left: 16px;">Next Step</wpp-button-v3-1-1>
      </div>
    </div>
  `
}
...
export const VerticalStepperWithWidth: Story<Props> = () => {
  const { handleNextClick, handlePreviousClick, handleStepClick, stepperState } =
    getVerticalStepperHandlers('with-width')
...
export const Horizontal: Story<Props> = (args: Props) => {
  const { handleHorizontalNextClick, handleHorizontalPreviousClick, handleHorizontalStepClick, stepperState } =
    getHorizontalStepperHandlers()
  ...
  return html`
    <div>
      <div style="display: flex; flex-direction: column;">
        <div style="width: 100%; height: 60px; text-align: center; margin-bottom: 20px;">
          ...
          >
            ...
            <wpp-step-v3-1-1 id="step" .error=${args.error} .iconDescription=${args.iconDescription}>
              <div slot="label">
                <p>Step 4</p>
              </div>
            </wpp-step-v3-1-1>
      ...
      <div style=${styleMap(buttonWrapper)}>
        <wpp-button-v3-1-1 .variant=${'secondary'} @click=${handleHorizontalPreviousClick}
          >Previous Step</wpp-button-v3-1-1
        >
        <wpp-button-v3-1-1 @click=${handleHorizontalNextClick} style="margin-left: 16px;">Next Step</wpp-button-v3-1-1>
      </div>
    </div>
  `
}
