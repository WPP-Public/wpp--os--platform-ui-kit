...
import { AvatarSize, Components } from '../../components'
...
const lineOptionMap: Record<string, (disabled?: boolean, size?: AvatarSize) => ReturnType<typeof html>> = {
  ...
  'with-action-btn': _disabled => html`
    <wpp-action-button-v3-1-1 variant="secondary" slot="right">
      <wpp-icon-plus-v3-1-1 slot="icon-start"></wpp-icon-plus-v3-1-1>
    </wpp-action-button-v3-1-1>
  `,
  'with-chevron': () => html`<wpp-icon-chevron-v3-1-1 slot="right"></wpp-icon-chevron-v3-1-1>`,
  'with-tag': disabled => html`
    <wpp-tag-v3-1-1 ?disabled="${disabled}" label="Text" variant="positive" slot="right"></wpp-tag-v3-1-1>
  `,
  'with-toggle': () => html`<wpp-toggle-v3-1-1 slot="right"></wpp-toggle-v3-1-1>`,
  'with-text': () => html`<wpp-typography-v3-1-1 slot="right" type="s-body">Text</wpp-typography-v3-1-1>`,

  // Icon left variants
  'with-icon': () => html`<wpp-icon-document-v3-1-1 slot="left"></wpp-icon-document-v3-1-1>`,
  'with-icon-and-action-btn': disabled => html`
    <wpp-icon-document-v3-1-1 slot="left"></wpp-icon-document-v3-1-1>
  ...
  'with-logo-and-tag': (disabled, size = 'xs') => html`
    ...
    <wpp-tag-v3-1-1 ?disabled="${disabled}" label="Text" variant="neutral" slot="right"></wpp-tag-v3-1-1>
  `,
  ...
  'with-logo-and-text': (disabled, size = 'xs') => html`
    ${lineOptionMap['with-logo'](disabled, size)}
    <wpp-typography-v3-1-1 slot="right" type="s-body" style="color:var(--wpp-grey-color-800)">
      Text
...
function getListItemContent(lineOption?: string, size: AvatarSize = 'xs', disabled?: boolean) {
  const templateFunc = lineOptionMap[lineOption || 'default'] || lineOptionMap['default']

  return templateFunc(disabled, size)
}

function getListItemRightContent(option?: string, disabled?: boolean) {
...
function getOneListItemOption(singleLineOption?: string, multiple?: boolean, disabled?: boolean) {
  return multiple
    ? getListItemRightContent(singleLineOption, disabled)
    : getListItemContent(singleLineOption, 'xs', disabled)
}

function getTwoListItemOption(twoLineOption?: string, multiple?: boolean, disabled?: boolean) {
  return multiple ? getListItemRightContent(twoLineOption, disabled) : getListItemContent(twoLineOption, 's', disabled)
}

export const SingleLine: Story<ListItemStoryArgsTypes> = (args: ListItemStoryArgsTypes) => html`
...
export const TwoLine: Story<ListItemStoryArgsTypes> = (args: ListItemStoryArgsTypes) => html`
  <wpp-list-item-v3-1-1
    .multiple="${args.multiple}"
    .nonInteractive="${args.nonInteractive}"
    .disabled="${args.disabled}"
    ...
    ?checked="${args.checked}"
  >
    ${args.subtitleText && args.subtitleText.length > 0 ? html`<span slot="subtitle">${args.subtitleText}</span>` : ''}
    ${getTwoListItemOption(args.twoLineOptions, args.multiple, args.disabled)}
    <p slot="label">${args.labelText}</p>
    <p slot="caption">${args.labelText}</p>
  </wpp-list-item-v3-1-1>
`
...
export const DynamicWidth: Story<ListItemStoryArgsTypes> = (args: ListItemStoryArgsTypes) => html`
  <Fragment>
    <wpp-typography-v3-1-1 type="xl-heading">Current width: ${args.width}</wpp-typography-v3-1-1>
    <wpp-list-item-v3-1-1
