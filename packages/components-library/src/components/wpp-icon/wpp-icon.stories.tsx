...
export const Icons = (args: WppIcon & { direction: string }) => {
      ...
      .map(
        ...
        ${
          section.groups?.length > 0
            ? `<wpp-typography-v3-1-1 type="xl-heading" tag="h3" style="display: flex; margin-top: 24px">
              ${section.title}
          ...
          ${
            section.groups?.length > 0
              ? section.groups
               ...
               <wpp-typography-v3-1-1 type="m-midi" tag="h4" style="display: flex;">
                 ${group.title}
               </wpp-typography-v3-1-1>
               ${group.icons
                 .map(icon => {
  ...
  const emptyState = () => {
    ...
    if (!isNothingFound) {
      return `<wpp-typography-v3-1-1 type='m-midi'>Nothing Found</wpp-typography-v3-1-1>`
    }
  ...
  return html`<div style=${styleMap(pageStyle)}>
    <div style=${styleMap(pageWrapper)}>
      <wpp-typography-v3-1-1 type="3xl-heading" tag="h3" style=${styleMap(headerStyle)}>Icons</wpp-typography-v3-1-1>
      ...
      <div style=${styleMap(actionsWrapper)}>
        <div style=${styleMap(actionItemStyle)}>
          ...
          <wpp-input-v3-1-1 type="search" @wppChange="${handleIconListChange}"></wpp-input-v3-1-1>
        </div>
      </div>
