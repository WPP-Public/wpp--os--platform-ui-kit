...
export const Tree: Story<Components.WppTree> = (args: Components.WppTree) => {
  const [treeState, setTreeState] = useState('treeState', data)
  const handleTreeChange = ({ detail }: any) => {
    setTreeState(detail.treeState)
  }

  return html`<wpp-tree-v3-1-1
...
export const TreeWithCustomSearch: Story<Components.WppTree> = (args: Components.WppTree) => {
  const [treeState, setTreeState] = useState('treeState', data)
  const handleTreeChange = ({ detail }: any) => {
    setTreeState(detail.treeState)
  }
  ...
  return html`
    <Fragment>
      <wpp-typography-v3-1-1 .type=${'l-strong'}>
...
TreeWithCustomSearch.args = {
  ...
  search: '',
  searchConfig: {
    isMatchingSearch: (item: TreeType, search: string) => item.title === search,
  },
}
