...
export const AvatarGroup: Story<Components.WppAvatarGroup> = (args: Components.WppAvatarGroup) => {
  const getData = () => {
    if (args.variant === 'circle') return users
    if (args.variant === 'square') return logos
  }

  return html` <wpp-avatar-group-v3-1-1
    .size="${args.size}"
    ...
    @wppSelectItem="${(e: any) => console.log('onWppSelectItem', e)}"
  />`
}
...
export const AvatarGroupInteractable: Story<Components.WppAvatarGroup> = (args: Components.WppAvatarGroup) => {
  const getData = () => {
    if (args.variant === 'circle') return interactableUsers
