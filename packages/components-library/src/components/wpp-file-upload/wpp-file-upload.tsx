...
export class WppFileUpload implements BaseFormControl<FileItemType[], FileUploadEventDetail> {
   ...
   */
  @Prop() readonly locales: FileUploadLocales = {
    label: 'Choose a file',
  ...
  private displayErrorListByShowingOption = (newFilesWithErrors: FileItemType[]): FileItemType[] =>
    this.showOnlyNewErrors ? newFilesWithErrors : [...this.errorList, ...newFilesWithErrors]
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        >
          <wpp-avatar class="icon-file" icon="wpp-icon-file" size="l" />
          ...
          <p class="text-info" part="text-info">
            {this.locales.info(this.getAcceptExtensions().join(', '), this.size)}
          </p>
          <input
            class="file-loader"
            type="file"
        ...
        {allFiles?.length ? (
          <div class={this.listWrapperCssClasses()} part="list-wrapper">
            <ul class="file-list" part="file-list" onScroll={this.handleListScroll}>
