...
import { AutocompleteExtendedOption, Components } from '../../components'
...
import { SlotItemNode } from '../wpp-list-item/types'
...
export default {
  ...
  parameters: {
    ...
    notes: { Autocomplete: AutocompleteReadme, Options: ListItemReadme },
  },
  argTypes: {
    messageType: {
...
} as Meta<typeof WppAutocomplete>
...
export const Regular: Story<AutocompleteStoryArgs> = (args: AutocompleteStoryArgs) => {
  const [value, setValue] = useState<Hero[]>('value', [])

  return html` <wpp-autocomplete-v3-1-1
  ...
  >
    <div slot="selected-values">
      ${value.map(
        selectedValue => html`
          ...
          >
            <wpp-avatar-v3-1-1 name="${selectedValue.label}" size="xs" src="${selectedValue!.url}" slot="icon-start" />
          </wpp-pill-v3-1-1>
        `,
      )}
    </div>
    ${heroes.map(
      hero => html`
        <wpp-list-item-v3-1-1 .value="${hero}">
          <wpp-avatar-v3-1-1 size="xs" src="${hero.url}" name="${hero.label}" slot="left"></wpp-avatar-v3-1-1>
          <p slot="label">${hero.label}</p>
        </wpp-list-item-v3-1-1>
      `,
    )}
  </wpp-autocomplete-v3-1-1>`
}
...
export const Extended: Story<AutocompleteStoryArgs> = (args: AutocompleteStoryArgs) => {
  const [value, setValue] = useState<Hero[]>('value', [])
...
export const RegularSlotSuggestions: Story<AutocompleteStoryArgs> = (args: AutocompleteStoryArgs) => {
  const [value, setValue] = useState<Hero[]>('value', [])
