...
export class WppSlider implements BaseComponent, BaseFormControl<SliderValue> {
   ...
   */
  @Prop() readonly maskOptions?: DecimalMaskOptions | DecimalMaskOptions[] = undefined
  ...
  onUpdateInputValue(newInputValue: SliderInputValue) {
    if (this.type === 'single') {
      const inputMaskOptions: DecimalMaskOptions = getMaskOptionsForInput(this.type, undefined, this.maskOptions)
      ...
      const minInputMaskOptions: DecimalMaskOptions = getMaskOptionsForInput(this.type, 'min', this.maskOptions)

      if (this.inputRef) {
        this.inputRef.value = formatDecimalWithMask(minValue, minInputMaskOptions)
      }

      const maxInputMaskOptions: DecimalMaskOptions = getMaskOptionsForInput(this.type, 'max', this.maskOptions)
  ...
  private handleInputChange = (type?: SliderRangeType) => (event: Event) => {
    ...
    const inputMaskOptions: DecimalMaskOptions = getMaskOptionsForInput(this.type, type, this.maskOptions)
  ...
  private handleBlur = (event: FocusEvent) => {
    ...
    const target = event.target as HTMLInputElement
    const type: SliderRangeType | null = target.classList.contains('min-input')
      ? 'min'
            ...
            maskOptions={{
              decimalPatternOptions: this.maskOptions
                ? {
                    ...
                    ...(this.maskOptions as DecimalMaskOptions),
                  }
                : getDefaultMaskOptions(this.step),
            }}
          />
        ...
        range: () => (
          <div class="range-input-wrapper" part="input-wrapper">
            <wpp-input
              ...
              maskOptions={{
                decimalPatternOptions:
                  this.maskOptions && (this.maskOptions as DecimalMaskOptions[])[0]
                    ? {
                        ...getDefaultMaskOptions(this.step),
                        ...(this.maskOptions as DecimalMaskOptions[])[0],
                      }
                    : getDefaultMaskOptions(this.step),
              }}
            />
            ...
            <wpp-input
              ...
              maskOptions={{
                decimalPatternOptions:
                  this.maskOptions && (this.maskOptions as DecimalMaskOptions[])[1]
                    ? {
                        ...getDefaultMaskOptions(this.step),
                        ...(this.maskOptions as DecimalMaskOptions[])[1],
                      }
                    : getDefaultMaskOptions(this.step),
              }}
            />
