...
type BannerStoryArgs = Components.WppBanner & { message: string; withActions: boolean }

export const NoTopBar: Story<BannerStoryArgs> = (args: BannerStoryArgs) => html` <wpp-banner-v3-1-1
  .show="${true}"
...
>
  ...
  ${args.withActions
    ? html`
        ${
          args.type === 'warning'
            ? html` <wpp-action-button-v3-1-1 variant="secondary" slot="actions">Action</wpp-action-button-v3-1-1>`
            : null
        }
        ${
          args.type === 'information'
            ? html` <wpp-action-button-v3-1-1 variant="inverted" slot="actions">Action</wpp-action-button-v3-1-1>`
            : null
        }
        </div>
      `
...
export const WithTopBar: Story<BannerStoryArgs> = (args: BannerStoryArgs) => {
  ...
  return html` <div style=${styleMap(styles.page)}>
    <div style=${styleMap(styles.header)}>
      <wpp-topbar-v3-1-1 .navigation="${initNavigation}" .value="${value}" @wppChange="${handleTopbarItemChange}">
        <div slot="app" style=${styleMap(styles.app)}>
          <wpp-typography-v3-1-1 type="m-strong" tag="h3" style=${styleMap(styles.appName)}>
            APP Name
    ...
    <div style=${styleMap(styles.container)}>
      ...
      <div style=${styleMap(styles.body)}>
        <div style=${styleMap(styles.section)}>
          <wpp-typography-v3-1-1 type="3xl-heading">Scrollable section</wpp-typography-v3-1-1>
          <div style=${styleMap(styles.actions)}>
            <wpp-button-v3-1-1 variant="secondary" @click="${handleShowBanner}"> Show Banner</wpp-button-v3-1-1>
            <wpp-button-v3-1-1 variant="primary" @click="${handleCloseBanner}"> Close Banner</wpp-button-v3-1-1>
          </div>
        </div>
