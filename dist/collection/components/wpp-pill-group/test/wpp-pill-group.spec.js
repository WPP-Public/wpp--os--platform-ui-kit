...
describe('wpp-pill-group', () => {
  ...
  it('renders component with users with prop src and color attributes', async () => {
    ...
    const page = await newSpecPage({
      components: [WppPillGroup, WppPill],
      template: () => (h("wpp-pill-group-v4-3-0", { size: "m", type: "multiple", value: value }, h("wpp-pill-v4-3-0", { name: "contact", value: "email", label: "Email" }), h("wpp-pill-v4-3-0", { name: "contact", value: "mail", label: "Mail" }), h("wpp-pill-v4-3-0", { name: "contact", value: "phone", label: "Phone" }))),
    });
    expect(page.root).toMatchSnapshot();
  });
});
