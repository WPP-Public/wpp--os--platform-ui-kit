...
describe('wpp-input', () => {
  ...
  describe('Testing componentWillLoad', () => {
    ...
    it('', async () => {
      ...
      const page = await newSpecPage({
        components: [WppInput],
        template: () => h("wpp-input-v3-6-0", null),
      });
  ...
  describe('Testing watchers', () => {
    ...
    it('Testing initial value updating the value property with no mask', async () => {
      const page = await newSpecPage({
        components: [WppInput],
        template: () => h("wpp-input-v3-6-0", { value: "initial" }),
      });
      const componentInstance = page.rootInstance;
      await page.waitForChanges();
    ...
    it('Testing initial value and updating the value property with mask', async () => {
      const page = await newSpecPage({
        components: [WppInput],
        template: () => (h("wpp-input-v3-6-0", { value: "0722334455", maskOptions: {
            telPatternOptions: {
              countryCode: 'RO',
            },
          }, type: "tel" })),
  ...
  it('renders component with label, icon and tooltip description', async () => {
    ...
    const page = await newSpecPage({
      ...
      template: () => h("wpp-input-v3-6-0", { labelConfig: labelConfig, name: "text-input" }),
    });
    await new Promise(resolve => setTimeout(resolve, 100));
    await page.waitForChanges();
    expect(page.root).toMatchSnapshot();
