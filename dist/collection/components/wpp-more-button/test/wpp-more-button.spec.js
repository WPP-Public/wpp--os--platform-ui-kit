...
describe('wpp-more-button', () => {
  ...
  describe('Testing snapshots', () => {
    it('Testing default button with name', async () => {
      const page = await newSpecPage({
        components: [WppMoreButton],
        template: () => h("wpp-more-button-v3-3-0", { name: "More items button", ariaProps: undefined }),
      });
      await page.waitForChanges();
    ...
    it('Testing button with aria-label', async () => {
      const page = await newSpecPage({
        components: [WppMoreButton],
        template: () => h("wpp-more-button-v3-3-0", { ariaProps: { label: 'More items menu' } }),
      });
      await page.waitForChanges();
      expect(page.root).toMatchSnapshot();
    });
    it('Testing button with aria-desribedby', async () => {
      const page = await newSpecPage({
        components: [WppMoreButton],
        template: () => h("wpp-more-button-v3-3-0", { ariaProps: { describedby: 'More items menu' } }),
      });
      await page.waitForChanges();
      expect(page.root).toMatchSnapshot();
    });
