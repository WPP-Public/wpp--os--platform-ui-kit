  ...
  describe('snapshots', () => {
    it('should render default component', async () => {
      const page = await newSpecPage({
        ...
        template: () => h("wpp-video-player-v4-3-0", { src: videoSrc }),
      });
    ...
    it('should render component with autoplay and mute', async () => {
      const page = await newSpecPage({
        components: [WppVideoPlayer],
        template: () => h("wpp-video-player-v4-3-0", { src: videoSrc, controlPanelConfig: { autoplay: true } }),
      });
      expect(page.root).toMatchSnapshot();
    });
  });
  describe('component', () => {
    describe('rendering', () => {
      ...
      it('should render with custom size', async () => {
        const page = await newSpecPage({
          components: [WppVideoPlayer],
          template: () => h("wpp-video-player-v4-3-0", { src: videoSrc, size: { width: 800, height: 600 } }),
        });
        const component = page.rootInstance;
      ...
      it('should render with multiple source elements when array is provided', async () => {
        ...
        const page = await newSpecPage({
          components: [WppVideoPlayer],
          template: () => (h("wpp-video-player-v4-3-0", { src: [videoSrc, videoSrc], type: ['video/mp4', 'video/webm'] })),
        });
      ...
      it('should create track elements when captions are provided', async () => {
        ...
        const page = await newSpecPage({
          components: [WppVideoPlayer],
          template: () => h("wpp-video-player-v4-3-0", { src: videoSrc, caption: captionData }),
        });
      ...
      it('should render without volume button and volume need to be set to 0', async () => {
        const page = await newSpecPage({
          components: [WppVideoPlayer],
          template: () => (h("wpp-video-player-v4-3-0", { src: videoSrc, controlPanelConfig: { showVolumeButton: false } })),
        });
