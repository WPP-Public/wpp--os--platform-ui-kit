...
export const Icons = {
  render: args => {
    ...
    const getIconsData = (args) => iconsDataList
      ...
      ? `<wpp-typography-v3-6-0 type="xl-heading" tag="h3" style="display: flex; margin-top: 24px">
              ${section.title}
      ...
      ? section.groups
        .map(group => `<div style='width: 100%; background: #F8F9FB; padding:16px; border-radius: 8px'>
               <wpp-typography-v3-6-0 type="m-midi" tag="h4" style="display: flex;">
                 ${group.title}
               </wpp-typography-v3-6-0>
               ${group.icons
        .map(icon => {
    ...
    const emptyState = () => {
      ...
      if (!isNothingFound) {
        return `<wpp-typography-v3-6-0 type='m-midi'>Nothing Found</wpp-typography-v3-6-0>`;
      }
      return '';
    };
    return html `<div style=${styleMap(pageStyle)}>
      <div style=${styleMap(pageWrapper)}>
        <wpp-typography-v3-6-0 type="3xl-heading" tag="h3" style=${styleMap(headerStyle)}>Icons</wpp-typography-v3-6-0>
        ...
        <div style=${styleMap(actionsWrapper)}>
          <div style=${styleMap(actionItemStyle)}>
            ...
            <wpp-input-v3-6-0 type="search" @wppChange="${handleIconListChange}"></wpp-input-v3-6-0>
          </div>
        </div>
