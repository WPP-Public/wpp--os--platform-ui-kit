...
const renderImageNode = (role, data, className) => {
  ...
  return data.linkHref ? (h("a", { href: data.linkHref, target: "_blank", rel: "noopener noreferrer", class: "chat-image-link" }, img)) : role === 'assistant' ? (h(Fragment, null, h("wpp-action-button-v4-2-0", { class: "image-download-button", variant: "secondary", onClick: () => handleDownload(data.href, data.alt) }, h("wpp-icon-download-v4-2-0", { slot: "icon-start", "aria-hidden": "true" })), img)) : (img);
};
export const renderToken = (token, role = 'user') => {
  switch (token.type) {
    ...
    case 'code':
      return (h("pre", { class: "code-block" }, h("div", { class: "code-lang" }, h("span", null, token.lang || 'plaintext'), h("wpp-icon-copy-v4-2-0", { class: "code-block-copy-icon", size: "s", color: "var(--wpp-grey-color-600)", onClick: () => {
    ...
    case 'list_item': {
      ...
      return (h("li", { class: `chat-list-item ${token.task ? `chat-list-item-task ${token.checked ? 'chat-list-item-task-checked' : ''}` : ''}` }, token.task && h("wpp-icon-tick-v4-2-0", null), inlineContent.length > 0 && (h("wpp-typography-v4-2-0", { type: "s-body" }, inlineContent.map(t => renderToken(t)))), nestedLists.map(t => renderToken(t))));
    }
    case 'table': {
      return (h("div", { class: "chat-table-wrapper" }, h("table", { class: "chat-table" }, h("thead", null, h("tr", null, token.header.map((cell, i) => (h("th", { key: i }, h("wpp-typography-v4-2-0", { type: "s-strong" }, cell.tokens?.map((t) => renderToken(t)) ?? cell.text)))))), h("tbody", null, token.rows.map((row, r) => (h("tr", { key: r }, row.map((cell, c) => (h("td", { key: c }, h("wpp-typography-v4-2-0", { type: "s-body" }, cell.tokens?.map((t) => renderToken(t)) ?? cell.text)))))))))));
    }
    case 'link':
      if (token.title) {
        return (h("wpp-tooltip-v4-2-0", { text: token.title }, h("a", { href: token.href, target: "_blank", rel: "noopener noreferrer", class: "chat-link" }, token.tokens?.length ? token.tokens.map((t) => renderToken(t)) : token.text)));
      }
