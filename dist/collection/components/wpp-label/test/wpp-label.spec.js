...
describe('wpp-label', () => {
  it('should render label with text and disabled state', async () => {
    ...
    const page = await newSpecPage({
      ...
      template: () => h("wpp-label-v3-6-0", { config: labelConfig, disabled: true, typography: "s-body" }),
    });
  ...
  it('should render label with optional text and icon', async () => {
    ...
    const page = await newSpecPage({
      components: [WppLabel, WppInternalLabel],
      template: () => h("wpp-label-v3-6-0", { config: labelConfig, typography: "s-body" }),
    });
    expect(page.root).toMatchSnapshot();
  });
  it('should render label with all text and icon with tooltip description ', async () => {
    ...
    const page = await newSpecPage({
      components: [WppLabel, WppInternalLabel],
      template: () => h("wpp-label-v3-6-0", { config: labelConfig, optional: true, typography: "s-body" }),
    });
    expect(page.root).toMatchSnapshot();
  });
});
