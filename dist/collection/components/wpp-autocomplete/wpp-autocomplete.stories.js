...
export const Regular = (args) => {
  ...
  return html ` <wpp-autocomplete-v2-22-0
  ...
  >
    <div slot="selected-values">
      ${value.map(selectedValue => html `
          ...
          >
            <wpp-avatar-v2-22-0 name="${selectedValue.label}" size="xs" src="${selectedValue.url}" slot="icon-start" />
          </wpp-pill-v2-22-0>
        `)}
    </div>
...
export const RegularSlotSuggestions = (args) => {
  const [value, setValue] = useState('value', []);
  return html ` <wpp-autocomplete-v2-22-0
    .loading="${args.loading}"
    .disabled="${args.disabled}"
