...
export function renderDropdownListComponent() {
  ...
  const renderListOptions = () => {
    ...
    return (h(Fragment, null, this.internalList.map((item, ndx) => {
      ...
      return (h("wpp-list-item-v4-1-0", { ref: el => (this.listItemsRefs[ndx] = el), onWppChangeListItem: this.handleClickListItem, key: convertValueToKey(item.value), ...rest, id: item.id !== undefined ? `${LIB_COMPONENTS_PREFIX}list-item-${item.id}` : undefined, role: "option" }, h("span", { slot: "label" }, label), slots && renderSlotsInListItem(slots, Boolean(label)).map((slotNode) => slotNode)));
    }), this.isInfiniteLoading && (h("div", { class: "wpp-dropdown-infinite-loader" }, h("wpp-spinner-v4-1-0", null)))));
  };
  const renderSuggestionOptions = () => {
    this.suggestionsItemsRefs = [];
    return (h(Fragment, null, h("wpp-typography-v4-1-0", { role: "presentation", type: "s-strong", class: "suggestions-heading" }, this._locales.suggestionTitle), this.componentSuggestions?.map((suggestion, ndx) => {
  ...
  if (this.loading) {
    return (h("wpp-list-item-v4-1-0", { role: "status", "non-interactive": true }, h("wpp-spinner-v4-1-0", { slot: "left" }), h("span", { slot: "label" }, this._locales.loading)));
  }
  if (!this.visibleOptionsLength && this.searchText.length > 0) {
    return (h("wpp-list-item-v4-1-0", { labelTypography: { color: 'var(--wpp-grey-color-700)', type: 's-body' }, nonInteractive: true }, h("wpp-typography-v4-1-0", { type: "s-body", class: "nothing-found", slot: "label" }, this._locales.nothingFound)));
  }
