...
export function renderDropdownPillsComponent() {
  ...
  return (h(Fragment, null, this.value.length > 0 && (h("div", { ref: ref => (this.headerWrapperRef = ref), class: {
    ...
    }, tabindex: "-1", onClick: () => this.setFocus() }, h("div", { ref: ref => (this.selectedPillsWrapperRef = ref), class: selectedPillsWrapperCssClasses() }, this.selectedOptions.map((option, ndx) => renderPillComponent(option, ndx, this.activePillsTruncationState[ndx]))), h("div", { ref: ref => (this.showMoreElementRef = ref), class: "show-more-action" }, showMoreLessRender(`+${this.activePillsTruncationState.filter(x => x).length} ${this._locales.showMore}`)))), !this.isShowMore && h("div", { class: "show-less-action" }, showMoreLessRender(this._locales.showLess)), isNeedDivider && h("wpp-divider-v4-3-0", { class: "nothing-found-divider" })));
}
