...
describe('wpp-select', () => {
  ...
  it('should render wpp-select with combined type with options list and with label, icon and tooltip description', async () => {
    ...
    const page = await newSpecPage({
      ...
      template: () => (h("wpp-select-v3-5-0", { placeholder: "Placeholder", type: "combined", labelConfig: labelConfig }, h("wpp-list-item-v3-5-0", { value: "car", "data-id": "1" }, h("span", { slot: "label" }, "Car")), h("wpp-list-item-v3-5-0", { value: "house", "data-id": "2", disabled: true }, h("span", { slot: "label" }, "House")), h("wpp-list-item-v3-5-0", { value: "apartment", "data-id": "3" }, h("span", { slot: "label" }, "Apartment")))),
    });
    await new Promise(resolve => setTimeout(resolve, 0));
    await page.waitForChanges();
    expect(page.root).toMatchSnapshot();
