...
const getEmptyStates = () => imageList.groups[0].images.map(image => html `
      <div style=${styleMap(itemWrapper)}>
        ...
        <wpp-typography-v4-1-0 type="xs-body" style=${styleMap(labelStyle)}>${image.label}</wpp-typography-v4-1-0>
      </div>
    `);
  ...
  render: () => html `
    <div style=${styleMap(pageStyle)}>
      <div style=${styleMap(pageWrapper)}>
        <wpp-typography-v4-1-0 type="3xl-heading" tag="h3" style=${styleMap(headerStyle)}>
          Empty States
