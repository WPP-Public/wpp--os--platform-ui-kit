...
describe('wpp-tree-item', () => {
  ...
  it('renders switcher when item.hasChildren is true (even without children)', async () => {
    const page = await newSpecPage({
      ...
      template: () => (h("wpp-tree-item-v4-3-0", { item: { ...baseItem, hasChildren: true, open: false }, level: 1, multiple: false, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
  ...
  it('sets aria-busy and disables action button when loadingChildren is true', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: { ...baseItem, hasChildren: true, open: true, loadingChildren: true }, level: 1, multiple: false, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
  ...
  it('onItemChange – open/loading/children/hidden branches', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => h("wpp-tree-item-v4-3-0", { item: baseItem, disableSearchHighlight: true }),
    });
  ...
  it('onSwitcherClick – stops propagation, sets timestamp, calls handleSwitcherClick', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => h("wpp-tree-item-v4-3-0", { item: baseItem }),
    });
    const instance = page.rootInstance;
  ...
  it('updateSlotData – sets icon slot flags correctly', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: baseItem }, h("span", { slot: "icon-start" }), h("span", { slot: "icon-end" }), h("div", { class: "wpp-menu-context" }))),
    });
    const instance = page.rootInstance;
  ...
  it('renderEndContent – renders text', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: baseItem, endContent: {
          contentType: 'text',
          props: { text: 'Hello' },
        } })),
    });
...
describe('wpp-tree-item accessibility', () => {
  ...
  it('renders with role="treeitem"', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: baseItem, level: 1, multiple: false, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
  ...
  it('renders with aria-label from item title', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: { ...baseItem, title: 'My Custom Title' }, level: 1, multiple: false, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
  ...
  it('renders with aria-level from level prop', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: baseItem, level: 3, multiple: false, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
  ...
  it('renders with aria-setsize and aria-posinset when provided', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: baseItem, level: 1, setSize: 5, posInSet: 2, multiple: false, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
  ...
  it('renders with aria-expanded for parent items (open)', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: { ...baseItem, hasChildren: true, open: true }, level: 1, multiple: false, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
  ...
  it('renders with aria-disabled when item is disabled', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: { ...baseItem, disabled: true }, level: 1, multiple: false, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
  ...
  it('renders with aria-selected for single-select mode', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: { ...baseItem, selected: true }, level: 1, multiple: false, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
  ...
  it('renders with aria-checked for multi-select mode', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: { ...baseItem, selected: true }, level: 1, multiple: true, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
  ...
  it('renders with aria-checked="mixed" for indeterminate state', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: { ...baseItem, indeterminate: true }, level: 1, multiple: true, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
  ...
  it('renders with is-focused attribute based on isFocused prop', async () => {
    const focusedPage = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: baseItem, level: 1, multiple: false, isFocused: true, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
    ...
    const notFocusedPage = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: baseItem, level: 1, multiple: false, isFocused: false, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
  ...
  it('does not render selection attributes for non-selectable items', async () => {
    const page = await newSpecPage({
      components: [WppTreeItem],
      template: () => (h("wpp-tree-item-v4-3-0", { item: { ...baseItem, isNotSelectable: true }, level: 1, multiple: false, search: "", highlightOptions: {}, disableOpenCloseAnimation: true })),
    });
  ...
  describe('subscribing to theme changes', () => {
    ...
    it('Test that the component does not subcribe to theme changes when the isDarkTheme prop is passed explicitly from the parent component', async () => {
      await newSpecPage({
        components: [WppTreeItem],
        template: () => h("wpp-tree-item-v4-3-0", { item: { ...baseItem }, isDarkTheme: true }),
      });
