...
export const NoTopBar = {
  render: args => html ` <wpp-banner-v3-4-0 .show="${true}" .closable="${args.closable}" .type="${args.type}">
      Warning message
      ${args.withActions
    ? html `
        ${args.type === 'warning'
      ? html ` <wpp-action-button-v3-4-0 variant="secondary" slot="actions">Action</wpp-action-button-v3-4-0>`
      : null}
        ${args.type === 'information'
      ? html ` <wpp-action-button-v3-4-0 variant="inverted" slot="actions">Action</wpp-action-button-v3-4-0>`
      : null}
        </div>
      `
    : null}
  ...
  render: args => {
    ...
    return html ` <div style=${styleMap(styles.page)}>
      <div style=${styleMap(styles.header)}>
        <wpp-topbar-v3-4-0 .navigation="${initNavigation}" .value="${value}" @wppChange="${handleTopbarItemChange}">
          <div slot="app" style=${styleMap(styles.app)}>
            <wpp-typography-v3-4-0 type="m-strong" tag="h3" style=${styleMap(styles.appName)}>
              APP Name
      ...
      ? html `
                ${args.type === 'warning'
        ? html `<wpp-action-button-v3-4-0 variant="secondary" slot="actions">
                      Action
      ...
      : null}
        ...
        <div style=${styleMap(styles.body)}>
          <div style=${styleMap(styles.section)}>
            <wpp-typography-v3-4-0 type="3xl-heading">Scrollable section</wpp-typography-v3-4-0>
            <div style=${styleMap(styles.actions)}>
              <wpp-button-v3-4-0 variant="secondary" @click="${handleShowBanner}"> Show Banner</wpp-button-v3-4-0>
              <wpp-button-v3-4-0 variant="primary" @click="${handleCloseBanner}"> Close Banner</wpp-button-v3-4-0>
            </div>
          </div>
