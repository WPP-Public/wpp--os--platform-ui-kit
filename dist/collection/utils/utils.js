...
export const getSlotEmptyStates = (nodes, slotSelectors) => {
  const emptySlots = {
    main: true,
  };
  if (slotSelectors) {
...
export const isEventTargetContained = (containerEl, event) => {
...
export const recursiveObjectMap = (initObj, mapFunction) => {
  const obj = { ...initObj };
  for (const k in obj) {
    if (isObject(obj[k])) {
...
export const getHighlightData = (initString = '', initSearch = '') => {
...
export const transformToVersionedTag = (tag) => {
...
export const applyBodyStylesIfNeeded = (action) => {
...
export const getDurationValues = (duration, componentDefaultValues) => duration === undefined
  ? [
  ...
  : typeof duration === 'number'
    ? [duration, duration]
    : duration;
export const selectDropdownWidth = (dropdownWidth, triggerEl, host) => {
...
export function getHasFocused() {
  return hasFocused;
}
export function setHasFocused(value) {
  hasFocused = value;
}
