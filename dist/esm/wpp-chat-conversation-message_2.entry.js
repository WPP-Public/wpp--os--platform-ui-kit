import { h, F as Fragment, r as registerInstance, H as Host, g as getElement, c as createEvent } from './index-9177bb6d.js';
import { m as marked_umd } from './marked.umd-be419d39.js';
import { k as transformToVersionedTag, c as hasParentWithId, g as getSlotEmptyStates, d as debounce } from './utils-f415b66e.js';
import { c as createCommonjsModule, a as commonjsGlobal } from './_commonjsHelpers-ba3f0406.js';
import { r as renderSlotsInListItem } from './utils-3d7b91ae.js';
import { L as LIB_COMPONENTS_PREFIX } from './const-ee6c5d3f.js';
import { c as convertMBToBytes, b as getExtensionsList, g as getExtension, E as EXTENSION_TO_TYPE, m as modifyPropertiesOnFile, a as getBaseName, r as renameFile } from './const-7de19053.js';
...
const renderImageNode = (role, data, className) => {
  const img = (h(Fragment, null,
    ...
    h("wpp-icon-image-v3-6-0", { role: "presentation", "aria-hidden": "true" })));
  return data.linkHref ? (h("a", { href: data.linkHref, target: "_blank", rel: "noopener noreferrer", class: "chat-image-link" }, img)) : role === 'assistant' ? (h(Fragment, null,
    h("wpp-action-button-v3-6-0", { class: "image-download-button", variant: "secondary", onClick: () => handleDownload(data.href, data.alt) },
      h("wpp-icon-download-v3-6-0", { slot: "icon-start", "aria-hidden": "true" })),
    img)) : (img);
};
const renderToken = (token, role = 'user') => {
  switch (token.type) {
    ...
    case 'heading': {
      ...
      return (h("wpp-typography-v3-6-0", { type: type, tag: tag }, token.tokens ? token.tokens.map(t => renderToken(t)) : token.text));
    }
    case 'del':
      return h("del", null, token.tokens?.map(t => renderToken(t)));
    case 'paragraph':
      return (h("wpp-typography-v3-6-0", { type: "s-body", tag: "p" }, token.tokens ? token.tokens.map(t => renderToken(t)) : token.text));
    case 'blockquote':
      return h("blockquote", { class: "chat-quote" }, token.tokens?.map(t => renderToken(t)));
    case 'strong':
      return (h("wpp-typography-v3-6-0", { type: "s-strong", tag: "span" }, token.tokens?.map(t => renderToken(t))));
    case 'em':
    ...
    case 'code':
      return (h("pre", { class: "code-block" },
        h("div", { class: "code-lang" },
          ...
          h("wpp-icon-copy-v3-6-0", { class: "code-block-copy-icon", size: "s", color: "var(--wpp-grey-color-600)", onClick: () => {
    ...
    case 'list_item': {
      ...
      return (h("li", { class: `chat-list-item ${token.task ? `chat-list-item-task ${token.checked ? 'chat-list-item-task-checked' : ''}` : ''}` },
        token.task && h("wpp-icon-tick-v3-6-0", null),
        inlineContent.length > 0 && (h("wpp-typography-v3-6-0", { type: "s-body" }, inlineContent.map(t => renderToken(t)))),
        ...
        h("table", { class: "chat-table" },
          h("thead", null,
            h("tr", null, token.header.map((cell, i) => (h("th", { key: i },
              h("wpp-typography-v3-6-0", { type: "s-strong" }, cell.tokens?.map((t) => renderToken(t)) ?? cell.text)))))),
          h("tbody", null, token.rows.map((row, r) => (h("tr", { key: r }, row.map((cell, c) => (h("td", { key: c },
            h("wpp-typography-v3-6-0", { type: "s-body" }, cell.tokens?.map((t) => renderToken(t)) ?? cell.text)))))))))));
    }
    case 'link':
      if (token.title) {
        return (h("wpp-tooltip-v3-6-0", { text: token.title },
    ...
    case 'hr':
      return h("wpp-divider-v3-6-0", null);
    case 'text':
      return token.tokens
    ...
    case 'tag':
      ...
      return null;
    default:
      return 'text' in token ? h("wpp-typography-v3-6-0", { type: "s-body" }, token.text) : null;
  }
};
...
const WppChatConversationMessage = class {
  constructor(hostRef) {
    ...
    this.renderActionButton = (data) => {
      if (!data.icon)
        return null;
      return (h("wpp-action-button-v3-6-0", { variant: "secondary", ...data }, h(transformToVersionedTag(data.icon), { slot: 'icon-start', part: 'icon' })));
    };
    this.renderMenuContextListItems = () => (h(Fragment, null, this.menuContextListItems.map(item => {
      const { label, slots, ...rest } = item;
      return (h("wpp-list-item-v3-6-0", { ...rest, id: item.id !== undefined ? `${LIB_COMPONENTS_PREFIX}list-item-${item.id}` : undefined, role: "option" }, h("span", { slot: "label" }, label), slots && renderSlotsInListItem(slots, Boolean(label)).map((slotNode) => slotNode)));
    })));
    this.hostCssClasses = () => ({
  ...
  renderComplete() {
    ...
    if (!Array.isArray(tokens)) {
      return (h("wpp-typography-v3-6-0", { type: "s-body", tag: "p" }, purify.sanitize(this.finalContent)));
    }
  ...
  renderAttachments() {
    ...
    return (h("div", { class: "attachments" }, images.length > 0 && (h("div", { class: "chat-image-grid-row" }, images.map(image => (h("div", { class: "chat-image-grid-item chat-image-error-wrap" }, h("img", { src: image.url, alt: image.name, loading: "lazy", onError: (e) => {
      ...
      } })))))), otherFiles && otherFiles.length > 0 && (h("div", { class: "chat-file-attachments-column" }, otherFiles.map(file => (h("wpp-file-upload-item-v3-6-0", { file: {
        name: file.name,
        url: file.url,
        type: file.type,
        size: file.size || 0,
  ...
  render() {
    return (h(Host, { class: this.hostCssClasses() }, h("div", { class: this.containerCssClasses() }, this.assistantAvatarConfig && (h("div", { class: "avatar-wrapper" }, this.role === 'assistant' && (h("wpp-avatar-v3-6-0", { size: "s", icon: "wpp-icon-ai", role: "presentation", ...this.assistantAvatarConfig })))), h("div", { class: this.contentCssClasses() }, h("div", { class: this.messageCssClasses() }, this.currentStatus === 'streaming' && this.renderStreaming(), this.currentStatus === 'complete' && this.renderComplete(), this.currentStatus === 'complete' && this.attachments.length > 0 && this.renderAttachments()), this.role === 'assistant' && this.status === 'complete' && (h("div", { class: "actions" }, h("div", { class: "action-toolbar" }, this._actionButtonConfig.map(this.renderActionButton), this.menuContextListItems && (h("wpp-menu-context-v3-6-0", null, h("wpp-action-button-v3-6-0", { variant: "secondary", slot: "trigger-element" }, h("wpp-icon-more-v3-6-0", { slot: "icon-start", direction: "horizontal" })), this.renderMenuContextListItems()))), this.sourcesActionConfig && (h("div", { class: "sources-action" }, h("wpp-action-button-v3-6-0", { variant: "secondary", ...this.sourcesActionConfig }, this.sourcesActionConfig.text)))))), this.userAvatarConfig && (h("div", { class: "avatar-wrapper" }, this.role === 'user' && h("wpp-avatar-v3-6-0", { size: "s", role: "presentation", ...this.userAvatarConfig }))))));
  }
