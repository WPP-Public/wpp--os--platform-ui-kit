...
import { N as Node3, m as mergeAttributes, w as wrappingInputRule, M as Mark, g as markInputRule, h as markPasteRule, i as TextSelection$1, S as Selection$1, j as textblockTypeInputRule, c as Plugin, P as PluginKey, k as canInsertNode, l as isNodeSelection, n as NodeSelection$1, o as nodeInputRule, p as combineTransactionSteps, q as getChangedRanges, u as findChildrenInRange, v as getMarksBetween, x as getAttributes, y as renderNestedMarkdownContent, e as Extension, z as getRenderedAttributes, A as parseIndentedBlocks, B as getNodeType, C as getNodeAtPosition, D as isNodeActive, F as isAtStartOfNode, G as isAtEndOfNode, H as Fragment$1, I as dropPoint, J as Slice$1, K as keydownHandler, L as DecorationSet, O as Decoration, Q as Mapping, U as callOrReturn, V as getExtensionField, W as createStyleTag, X as isNodeEmpty, Y as getStyleProperty, Z as SelectionRange$1, _ as Transform, $ as findParentNodeClosestToPos, a0 as textInputRule, a1 as commands_exports, a2 as sortExtensions, a3 as flattenExtensions, a4 as decodeHtmlEntities, a5 as marksEqual, a6 as generateJSON, a7 as getSchema, a8 as encodeHtmlEntities, a9 as attrsEqual, aa as ResizableNodeView, ab as InputRule } from './index-90a07263.js';
...
function normalizeSelection(state, tr, allowTableNodeSelection) {
	...
	} else if (sel instanceof TextSelection$1 && isCellBoundarySelection(sel)) normalize = TextSelection$1.create(doc, sel.from);
	else if (sel instanceof TextSelection$1 && isTextSelectionAcrossCells(sel)) normalize = TextSelection$1.create(doc, sel.$from.start(), sel.$from.end());
...
function deleteCellSelection(state, dispatch) {
	...
	if (dispatch) {
		...
		sel.forEachCell((cell, pos) => {
			if (!cell.content.eq(baseContent)) tr.replace(tr.mapping.map(pos + 1), tr.mapping.map(pos + cell.nodeSize - 1), new Slice$1(baseContent, 0, 0));
		});
...
function pastedCells(slice) {
	...
	else if (role == "cell" || role == "header_cell") rows.push(openStart || openEnd ? fitSlice(tableNodeTypes(schema).row, new Slice$1(content, openStart, openEnd)).content : content);
	else return null;
...
function insertCells(state, dispatch, tableStart, rect, cells) {
	...
	for (let row = top; row < bottom; row++) {
		...
		tr.replace(tr.mapping.slice(mapFrom).map(from + tableStart), tr.mapping.slice(mapFrom).map(to + tableStart), new Slice$1(cells.rows[row - top], 0, 0));
	}
	recomp();
...
export { TIPTAP_DEFAULT_TOOLBAR_ALIASES as T, normalizeListHtml as a, buildTiptapExtensions as b, extractPlainText as e, normalizeEmptyParagraphs as n };
