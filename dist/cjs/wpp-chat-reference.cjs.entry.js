...
const utils$1 = require('./utils-fb205c59.js');
...
const WppChatReference = class {
  ...
  renderThumbnail() {
    if (this.src) {
      return index.h("img", { src: this.src, alt: "", class: "thumbnail-image", part: "thumbnail-image" });
    }
    const extension = this.fileExtension || utils$1.getExtension(this.name || '');
    return utils$1.returnIconFromExtension(extension, null);
  }
  renderFile() {
    return (index.h(index.Fragment, null, index.h("div", { class: "thumbnail", part: "thumbnail" }, this.renderThumbnail()), index.h("div", { class: "details", part: "content" }, index.h("wpp-typography-v4-2-0", { class: "name", type: "xs-midi", part: "name" }, this.name), this.fileType && (index.h("wpp-typography-v4-2-0", { class: "type", type: "xs-body", part: "type" }, this.fileType)))));
  }
  renderText() {
    return (index.h("div", { class: "text-content", part: "content" }, index.h("wpp-typography-v4-2-0", { class: { text: true, [`lines-${this.lines}`]: true }, type: "xs-body", part: "text" }, this.text)));
  }
  render() {
    return (index.h(index.Host, null, index.h("div", { class: "reference", part: "reference" }, index.h("span", { class: "block-line", part: "block-line", "aria-hidden": "true" }), this.type === 'file' ? this.renderFile() : this.renderText(), this.removable && (index.h("wpp-icon-cross-v4-2-0", { class: "close", part: "close", role: "button", tabindex: 0, "aria-label": this._locales.removeLabel, onClick: this.handleClose, onKeyDown: this.handleCloseKeyDown })))));
  }
  static get registryIs() { return "wpp-chat-reference-v4-2-0"; }
};
WppChatReference.style = wppChatReferenceCss;

exports.wpp_chat_reference = WppChatReference;
