...
const WppTree = class {
  constructor(hostRef) {
    ...
    this.multipleSelectionUpdate = (tree, item) => {
      ...
      this.wppChange.emit({
        treeState,
        currentItem: item,
        selectedItems: utils.findSelectedItems(treeState),
    ...
    this.renderIconsList = (item, icons, place = 'end') => (index.h("div", { slot: `icon-${place}`, key: utils$1.uuidv4() }, index.h("wpp-menu-context-v2-22-0", { appendToListWrapper: true, dropdownConfig: {
        popperOptions: { strategy: 'fixed' },
        trigger: 'click',
        interactiveDebounce: 15,
        interactiveBorder: 25,
        offset: [0, 0],
      } }, index.h("wpp-icon-more-v2-22-0", { class: {
        'menu-trigger': true,
        disabled: !!item.disabled,
      }, style: { padding: '4px', color: 'var(--wpp-grey-color-800)' }, direction: "horizontal", slot: "trigger-element" }), index.h("div", null, icons.map(({ icon, name }) => (index.h("wpp-list-item-v2-22-0", { key: name, value: name, onClick: this.handleActionClick({ item, name, place }) }, index.h(utils$1.transformToVersionedTag(icon), { slot: 'left' }), index.h("p", { slot: "label" }, name))))))));
    this.renderTree = (treeData, level = 1) => treeData.map(item => {
      if (item.children) {
        return (index.h("wpp-tree-item-v2-22-0", { text: item.title, item: item, level: level, multiple: this.multiple, search: this.search, highlightOptions: this.searchConfig.highlightOptions, transformSearchQuery: this.searchConfig.transformSearchQuery, disableSearchHighlight: this.disableSearchHighlight, disableOpenCloseAnimation: this.disableOpenCloseAnimation, withItemsTruncation: this.withItemsTruncation, endContent: item.endContent }, item.iconStart?.icon &&
    ...
    this.renderSkeletonLoading = () => [...Array(this.skeletonNumberItems)].map(_ => (index.h("div", { class: "skeleton-item" }, index.h("wpp-skeleton-v2-22-0", { variant: "rectangle", width: "100%", animation: true }))));
  ...
  render() {
    return (index.h(index.Host, { class: this.hostCssClasses(), exportparts: "tree-container, tree-empty-text" }, !this.loading && (index.h("div", { class: "container", part: "tree-container" }, this.currentTreeData && this.isSearchResultFound ? (this.renderTree(this.currentTreeData)) : (index.h("p", { class: "empty-tree-text", part: "tree-empty-text" }, this.locales.nothingFound)))), this.loading && index.h("div", { class: "skeleton-wrapper" }, this.renderSkeletonLoading())));
  }
