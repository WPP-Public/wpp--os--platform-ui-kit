...
const WppPill = class {
  constructor(hostRef) {
    ...
    this.renderLabel = () => {
      ...
      return shouldShowTooltip ? (index.h("wpp-tooltip-v4-2-0", { text: originalLabel, disabled: this.disabled }, labelNode)) : (labelNode);
    };
    this.setLabelRef = (el) => {
      if (el === this.labelRef)
        return;
  ...
  render() {
    return (index.h(index.Host, { class: this.hostCssClasses(), "aria-disabled": this.disabled, "aria-checked": this.checked ? 'true' : 'false', "aria-hidden": this.disabled ? 'true' : null, onClick: this.onClick, onFocus: this.onFocus, onBlur: this.onBlur, onMouseDown: this.onMouseDown, onKeyUp: (event) => this.onKeyUp(event, 'wrapper'), role: "checkbox", exportparts: "input, pill-wrapper, drag-wrapper, drag-icon, label, inner, active-icon, remove-icon, icon-start, icon-start-wrapper", tabIndex: this.checkTabIndex() }, index.h("input", { class: "pill-input", type: "checkbox", name: this.name, disabled: this.disabled, ref: focusEl => (this.inputEl = focusEl), "aria-label": this.ariaProps.label, "aria-hidden": "true", part: "input", tabIndex: -1 }), index.h("div", { class: this.cssClasses(), part: "pill-wrapper" }, this.type === 'draggable' ? (index.h("div", { class: this.slotCssClasses(), part: "drag-wrapper" }, index.h("wpp-icon-drag-v4-2-0", { class: { [`${this.focusType['icon-draggable']}`]: true }, part: "drag-icon", onMouseEnter: () => this.updateComponentState('hover'), onMouseLeave: () => this.updateComponentState(null), onMouseDown: ev => {
        this.updateComponentState('active');
        this.onDragPress(ev);
        this.onMouseDown();
      }, onMouseUp: () => this.updateComponentState(null), tabIndex: this.disabled ? -1 : 0, onKeyUp: (event) => this.onKeyUp(event, 'icon-draggable') }))) : (index.h(WrappedSlot.WrappedSlot, { name: "icon-start", wrapperClass: this.slotCssClasses(), onSlotchange: this.updateSlotData })), this.renderLabel(), this.checked && this.type === 'multiple' && index.h("wpp-icon-tick-v4-2-0", { class: "active-icon", part: "active-icon" }), this.removable && (this.type === 'display' || this.type === 'draggable') && (index.h("wpp-icon-cross-v4-2-0", { class: { [`${this.focusType['icon-close']}`]: true }, part: "remove-icon", onClick: this.onClose, tabIndex: this.disabled ? -1 : 0, onMouseDown: this.onMouseDown, onKeyUp: (event) => this.onKeyUp(event, 'icon-close') })))));
  }
  static get registryIs() { return "wpp-pill-v4-2-0"; }
