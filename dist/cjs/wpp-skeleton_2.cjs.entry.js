...
const utils$1 = require('./utils-ce02f074.js');
...
const WppSkeleton = class {
  ...
  static get registryIs() { return "wpp-skeleton-v4-3-0"; }
};
...
const WppTreeItem = class {
  constructor(hostRef) {
    ...
    this.releaseAncestorHeights = () => {
      ...
      const TAG = utils.transformToVersionedTag('wpp-tree-item').toUpperCase();
      while (el) {
        if (el.tagName === TAG) {
    ...
    this.renderEndContent = () => {
      ...
      switch (contentType) {
        case 'text':
          return (index.h("wpp-typography-v4-3-0", { type: "s-body", tag: "span", ...props, class: this.endContentCssClasses(className), part: "tree-item-end-text" }, props?.text));
        case 'tag': {
          const { icon } = props;
          return (index.h("wpp-tag-v4-3-0", { ...props, class: this.endContentCssClasses(className), disabled: this.item.disabled, part: "tree-item-end-tag" }, icon &&
        ...
        case 'avatar':
          return (index.h("wpp-avatar-v4-3-0", { ...props, class: this.endContentCssClasses(className), size: "xs", part: "tree-item-end-avatar",
            // Remove from tab order - tree uses arrow keys per ARIA APG
            index: -1 }));
        case 'avatarGroup':
          return (index.h("wpp-avatar-group-v4-3-0", { ...props, class: this.endContentCssClasses(className), part: "tree-item-end-avatar-group",
  ...
  render() {
    ...
    return (index.h(index.Host, { class: this.hostCssClasses(), exportparts: "tree-item,tree-item-switcher,tree-item-checkbox,tree-item-title-wrapper,tree-item-title,tree-item-title-highlighted,tree-item-action-button", role: "treeitem", "aria-label": this.item.title, "aria-level": this.level, "aria-setsize": this.setSize, "aria-posinset": this.posInSet, "aria-expanded": isParent ? (this.item.open ? 'true' : 'false') : undefined, "aria-disabled": this.item.disabled ? 'true' : undefined, "aria-busy": this.item.loadingChildren ? 'true' : undefined, ...(this.isDarkTheme !== undefined ? { 'data-wpp-theme': this.isDarkTheme ? 'dark' : 'light' } : {}), ...selectionAttr, ...(!this.disableOpenCloseAnimation && { onTransitionEnd: this.handleTransitionEnd }) }, index.h("div", { class: this.treeItemClasses(), style: { paddingLeft: this.calculateItemOffset(this.level, isParent) }, onClick: this.handleItemClick, part: "tree-item" }, isParent && (index.h("div", { class: "switcher", onClick: this.onSwitcherClick, part: "tree-item-switcher", "data-switcher": "true", "aria-hidden": "true" }, index.h("wpp-icon-triangle-fill-v4-3-0", { "data-open": this.item.open ? 'true' : 'false' }))), this.multiple && !this.item.isNotSelectable && (index.h("wpp-checkbox-v4-3-0", { class: "checkbox", indeterminate: this.item.indeterminate, checked: this.item.selected, controlled: true, onWppChange: this.handleCheckboxClick, disabled: this.item.disabled, part: "tree-item-checkbox", isDarkTheme: this.isDarkTheme, "aria-hidden": "true", index: -1 })), index.h(WrappedSlot.WrappedSlot, { name: "icon-start", onSlotchange: this.updateSlotData, wrapperClass: this.iconStartCssClasses() }), this.isTextWrappable && this.withItemsTruncation ? (index.h("wpp-tooltip-v4-3-0", { text: this.item.title, config: { placement: 'right' }, class: "tooltip", anchorTabIndex: -1 }, this.renderTitle())) : (this.renderTitle()), index.h("wpp-action-button-v4-3-0", { variant: "secondary", disabled: this.item.disabled || this.item.loadingChildren, onMouseEnter: this.handleMouseDown, onMouseLeave: this.handleMouseLeave, class: this.iconEndCssClasses(), loading: this.item.loadingActions, part: "tree-item-action-button", ariaProps: {
        ...
        tabIndex: -1,
      }, "aria-hidden": !this.hasIconEndSlot ? 'true' : undefined }, index.h("slot", { name: "icon-end", onSlotchange: this.updateSlotData })), this.renderEndContent()), ((this.item.open &&
      (this.item.loadingChildren || (Array.isArray(this.item.children) && this.item.children.length > 0))) ||
      !this.isCollapseTransitionEnd) && index.h(WrappedSlot.WrappedSlot, { name: "content", onSlotchange: this.updateSlotData })));
  }
