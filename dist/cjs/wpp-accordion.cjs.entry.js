...
const WppAccordion = class {
  constructor(hostRef) {
    ...
    this.onBlur = (event) => {
      ...
      this.focusType = this.getUpdatedFocusInfo('slot', common.FOCUS_TYPE.NONE);
      this.wppBlur.emit(event);
    };
    this.onMouseDown = () => {
      this.focusType = this.getUpdatedFocusInfo('wrapper', common.FOCUS_TYPE.MOUSE);
      this.focusType = this.getUpdatedFocusInfo('slot', common.FOCUS_TYPE.MOUSE);
    };
    ...
    this.cssClasses = () => ({
      ...
      'tab-focus': this.focusType.wrapper === common.FOCUS_TYPE.TAB && this.focusType.slot !== common.FOCUS_TYPE.TAB,
      [`size-${this.size}`]: true,
    });
  ...
  render() {
    ...
    const titleContent = (index.h(index.Fragment, null, index.h("wpp-typography-v2-22-0", { type: this.typographyType(), part: "title", class: "title-text" }, this.text), index.h(WrappedSlot.WrappedSlot, { wrapperClass: this.headerCssClasses(), name: "header", onSlotchange: this.updateSlotData })));
    return (index.h(index.Host, { class: this.hostCssClasses(internal), "aria-expanded": this.expanded, role: "treeitem", exportparts: "section, title, icon, counter, divider, title-wrapper, content", onFocus: this.onFocus, onBlur: this.onBlur, onMouseDown: this.onMouseDown, "aria-disabled": this.disabled, onKeyUp: (event) => this.onKeyUp(event, 'wrapper'), style: style, tabIndex: this.tabIndex }, index.h("div", { class: this.cssClasses(), part: "section" }, index.h("div", { class: "title-tags-wrapper", onClick: this.onClick }, index.h("div", { class: "title-wrapper", part: "title-wrapper" }, index.h("wpp-icon-chevron-v2-22-0", { part: "icon" }), this.isTitleOverflowing ? index.h("wpp-tooltip-v2-22-0", { text: tooltipText }, titleContent) : titleContent, this.counter > 0 && (index.h("wpp-typography-v2-22-0", { type: this.counterType(), class: "counter", part: "counter" }, `(${this.counter})`))), this.withTag && (index.h(WrappedSlot.WrappedSlot, { wrapperClass: this.tagGroupCssClasses(), name: "tags", onSlotchange: this.updateSlotData }))), index.h(WrappedSlot.WrappedSlot, { wrapperClass: this.actionsCssClasses(), name: "actions", onSlotchange: this.updateSlotData })), index.h("div", { class: this.contentCssClasses(), part: "content", onMouseDown: this.onMouseDown, onKeyUp: (event) => this.onKeyUp(event, 'slot') }, index.h("slot", { class: "slot-content" })), this.withDivider && index.h("wpp-divider-v2-22-0", { part: "divider" })));
  }
