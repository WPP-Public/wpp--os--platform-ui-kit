...
const WppAccordion = class {
  constructor(hostRef) {
    ...
    this.updateSlotData = () => {
      ...
      if (!emptyStates.tags && this.disabled) {
        const tagEl = this.host?.querySelector(utils.transformToVersionedTag('wpp-tag').toUpperCase());
    ...
    this.onKeyDown = (event) => {
      if (this.disabled || this.focusType === common.FOCUS_TYPE.NONE)
        return;
  ...
  render() {
    ...
    const titleContent = (index.h(index.Fragment, null, this.hasHeaderSlot && (index.h("wpp-typography-v4-3-0", { class: "typography-title", part: "title", type: this.getHeaderTypographyType(), style: { '--typography-color': 'var(--wpp-text-color)' } }, index.h(WrappedSlot.WrappedSlot, { wrapperClass: this.headerCssClasses(), name: "header", role: "presentation", onSlotchange: this.updateSlotData })))));
    return (index.h(index.Host, { class: this.hostCssClasses(internal), exportparts: "section, title, icon, divider, title-wrapper, content, header-wrapper, typography", style: style, onBlur: this.onBlur, onKeyUp: this.onKeyUp, onKeyDown: this.onKeyDown }, index.h("div", { class: this.cssSectionClasses(), part: "section" }, index.h("button", { ref: ref => (this.titleTagsWrapperButtonRef = ref), class: this.cssTagWrapperClasses(), "aria-expanded": this.expanded.toString(), "aria-controls": this.ariaProps?.controls ?? 'expandable-panel', id: this.ariaProps?.labelledby ?? 'expandable-button', onFocus: this.onFocus, onClick: this.onClick, disabled: this.disabled, part: "button" }, index.h("div", { class: "title-wrapper", part: "title-wrapper", role: "none" }, index.h("wpp-icon-chevron-v4-3-0", { role: "presentation", part: "icon" }), this.isTitleOverflowing ? (index.h("wpp-tooltip-v4-3-0", { config: { triggerTarget: this.titleTagsWrapperButtonRef }, text: tooltipText }, titleContent)) : (titleContent)), this.withTag && (index.h(WrappedSlot.WrappedSlot, { wrapperClass: this.tagGroupCssClasses(), name: "tags", onSlotchange: this.updateSlotData }))), index.h(WrappedSlot.WrappedSlot, { wrapperClass: this.actionsCssClasses(), name: "actions", onSlotchange: this.updateSlotData })), index.h("div", { class: this.contentCssClasses(), part: "content", role: "region", id: this.ariaProps?.controls ?? 'expandable-panel', "aria-labelledby": this.ariaProps?.labelledby ?? 'expandable-button', ...(!this.expanded ? { inert: true, 'aria-hidden': true } : {}) }, index.h("slot", { class: "slot-content" })), this.withDivider && index.h("wpp-divider-v4-3-0", { part: "divider" })));
  }
  static get registryIs() { return "wpp-accordion-v4-3-0"; }
...
exports.wpp_accordion = WppAccordion;
