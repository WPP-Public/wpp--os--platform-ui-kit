...
const WppAvatarGroup = class {
    ...
    this.getAvatarsWithColors = (avatars) => {
      ...
      return avatars.map(avatar => {
        if (!avatar.src && !avatar.color) {
          avatar.color = consts.AVATAR_COLORS_VARIANTS[colorIndex];
          colorIndex = colorIndex === consts.AVATAR_COLORS_VARIANTS.length ? 0 : colorIndex + 1;
        }
        return avatar;
      });
    };
  ...
  render() {
    ...
    return (index.h(index.Host, { class: this.avatarGroupWrapperCssClasses(), exportparts: "list, item, menu, avatar, hidden-item, hidden-item-with-avatar, hidden-item-avatar, hidden-item-name" }, index.h("ul", { class: "avatars-list", part: "list" }, avatarsToDisplay.map((avatar, avatarIndex) => (index.h("li", { class: {
        'avatar-item': true,
        interactable: avatar.interactable ?? false,
      }, part: "item" }, index.h("wpp-avatar-v2-22-0", { size: this.size, variant: this.variant, name: avatar.name, src: avatar.src, withTooltip: this.withTooltip, tooltipConfig: this.tooltipConfig, interactable: avatar.interactable, color: avatar.color, onWppClick: (event) => this.handleAvatarClick(event, avatarIndex, false) })))), this.getAvatarsList().length > this.maxAvatarsToDisplay && (index.h("li", { class: "avatar-item", part: "item" }, index.h("wpp-menu-context-v2-22-0", { externalClass: "avatar-group", listWidth: "240px", dropdownConfig: this.dropdownConfig, part: "menu" }, index.h("wpp-avatar-v2-22-0", { amountOfHiddenAvatars: avatarsInHiddenList.length, size: this.size, variant: this.variant, slot: "trigger-element", part: "avatar" }), index.h("div", null, avatarsInHiddenList.map((avatar, avatarIndex) => (index.h("wpp-list-item-v2-22-0", { key: avatar.name, onWppChangeListItem: (event) => this.handleListItemClick(event, avatarIndex), value: avatar.name, part: "hidden-item" }, index.h("wpp-avatar-v2-22-0", { name: avatar.name, src: avatar.src, color: avatar.color, variant: this.variant, interactable: avatar.interactable, index: -1, slot: "left", part: "hidden-item-avatar" }), index.h("span", { slot: "label", class: "name", part: "hidden-item-name" }, avatar.name)))))))))));
  }
...
const WppTag = class {
  ...
  render() {
    return (index.h(index.Host, { class: this.hostCssClasses(), exportparts: "label, tooltip, tooltip-text, icon-start, overlay" }, index.h(WrappedSlot.WrappedSlot, { wrapperClass: this.iconStartCssClasses(), name: "icon-start", onSlotchange: this.updateSlotData }), index.h("wpp-typography-v2-22-0", { type: "xs-midi", tag: "span", part: "label" }, Number(this.label?.length) > this.maxLabelLength ? (index.h("wpp-tooltip-v2-22-0", { text: this.label, config: this.tooltipConfig, part: "tooltip" }, index.h("span", { part: "tooltip-text" }, utils.truncate(this.label, this.maxLabelLength, false)))) : (this.label)), index.h("div", { class: "overlay", part: "overlay" })));
  }
