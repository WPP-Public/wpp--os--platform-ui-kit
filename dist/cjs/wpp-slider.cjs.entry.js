  ...
  constructor(hostRef) {
    ...
    this.applyTruncationToMarks = () => {
      requestAnimationFrame(() => {
        ...
        const marks = this.marksListRef?.querySelectorAll(`${utils.transformToVersionedTag('wpp-typography')}[part="label"]`);
        if (!marks)
          return;
    ...
    this.handleInputBlur = (type) => {
      this.focusType = this.getUpdatedFocusInfo(type, common.FOCUS_TYPE.NONE);
    };
    this.handleInputMouseDown = (type) => {
      this.focusType = this.getUpdatedFocusInfo(type, common.FOCUS_TYPE.MOUSE);
    };
    ...
    this.renderControl = () => {
      const label = this.labelConfig?.text && (index.h("wpp-label-v2-22-0", { htmlFor: this.name, optional: !this.required, disabled: this.disabled, config: this.labelConfig, tooltipConfig: this.labelTooltipConfig, part: "label" }));
      if (this.withValue && !this.withInput) {
        return (index.h("div", { class: this.controlCssClasses(), part: "control-wrapper" }, label, this.handleType({
          single: value => (index.h("wpp-typography-v2-22-0", { type: "s-midi", part: "value" }, value)),
          range: value => (index.h("div", { class: "range-value-wrapper", part: "value-wrapper" }, index.h("wpp-typography-v2-22-0", { type: "s-midi", part: "value" }, value[0]), index.h("wpp-divider-v2-22-0", { part: "value-divider", class: { divider: true, disabled: this.disabled } }), index.h("wpp-typography-v2-22-0", { type: "s-midi", part: "value" }, value[1]))),
        })));
      }
      return label;
    };
    this.renderEditableInput = () => (index.h("div", { class: this.editableInputCssClasses(), part: "editable-input-wrapper" }, this.handleType({
      single: () => (index.h("wpp-input-v2-22-0", { ref: inputRef => (this.inputRef = inputRef), type: "decimal", disabled: this.disabled, part: "input-number", onBlur: this.handleBlur, onFocus: this.handleFocus, style: { width: this.inputWidth ? this.inputWidth : DEFAULT_INPUT_WIDTH }, class: { [`size-${this.size}`]: true }, maskOptions: {
      ...
      range: () => (index.h("div", { class: "range-input-wrapper", part: "input-wrapper" }, index.h("wpp-input-v2-22-0", { ref: inputRef => (this.inputRef = inputRef), type: "decimal", disabled: this.disabled, part: "input-min", onBlur: this.handleBlur, onFocus: this.handleFocus, style: { width: this.inputWidth ? this.inputWidth : DEFAULT_INPUT_WIDTH }, class: { 'min-input': true, [`size-${this.size}`]: true }, maskOptions: {
          decimalPatternOptions: this.maskOptions && this.maskOptions[0]
            ...
            : getDefaultMaskOptions(this.step),
        } }), index.h("wpp-divider-v2-22-0", { class: { 'wpp-disabled': this.disabled }, part: "divider" }), index.h("wpp-input-v2-22-0", { ref: inputRef => (this.inputMaxRef = inputRef), type: "decimal", disabled: this.disabled, part: "input-max", onBlur: this.handleBlur, onFocus: this.handleFocus, style: { width: this.inputWidth ? this.inputWidth : DEFAULT_INPUT_WIDTH }, class: { 'max-input': true, [`size-${this.size}`]: true }, maskOptions: {
    ...
    this.renderMarks = () => {
      if (this.displayMarks.length > 0) {
        ...
        return this.displayMarks.map((mark, index$1) => {
          const dynamicOffset = calculateDynamicOffset(index$1, totalMarks);
          const isFirstMark = index$1 === 0;
          const isLastMark = index$1 === totalMarks - 1;
          const style = {
          ...
          const labelContent = (index.h("wpp-typography-v2-22-0", { id: `mark-label-${mark.value}`, class: this.labelCssClasses(), type: "xs-body", part: "label" }, labelText));
          return (index.h("div", { onClick: event => this.handleMarkClick(event, mark), class: this.markCssClasses(mark.value), style: style, part: "mark" }, !this.continuous && (index.h("div", { class: "circle", part: "mark-circle" }, index.h("div", { class: "mark", part: "mark-inner" }))), index.h("div", { class: "label-container" }, isTruncated ? (index.h("wpp-tooltip-v2-22-0", { config: { placement: tooltipPlacement }, text: this.tooltipTexts[mark.value] }, labelContent)) : (labelContent))));
        });
      }
    };
    this.tooltipTexts = {};
...
WppSlider.style = wppSliderCss;

exports.wpp_slider = WppSlider;
