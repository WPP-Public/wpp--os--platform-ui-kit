...
const WppAvatar = class {
  ...
  componentWillLoad() {
    if (!this.isAvatarIcon()) {
      this.colorChange(this.color || consts.AVATAR_COLORS_VARIANTS[Math.floor(Math.random() * consts.AVATAR_COLORS_VARIANTS.length)]);
    }
  }
  render() {
    const content = this.src && !this.isImageFailedToLoad ? (index.h("div", { class: this.imageWrapperCssClasses(), part: "content" }, index.h("img", { src: this.src, alt: this.name, class: this.imageCssClasses(), onError: this.handleImageLoadFailure, part: "image" }))) : (index.h(index.Fragment, null, index.h("div", { class: this.contentWrapperCssClasses(), part: "content" }, this.amountOfHiddenAvatars ? `+${this.amountOfHiddenAvatars}` : this.getUserAbbreviation(this.name), this.renderIcon())));
    return (index.h(index.Host, { class: this.hostCssClasses(), onBlur: this.onBlur, onMouseDown: this.onMouseDown, onKeyUp: this.onKeyUp, onClick: this.handleClick, exportparts: "image, content, tooltip", tabIndex: this.index }, this.withTooltip && !this.isAvatarIcon() ? (index.h("wpp-tooltip-v2-22-0", { text: this.name, config: this.tooltipConfig, part: "tooltip" }, content)) : (content)));
  }
