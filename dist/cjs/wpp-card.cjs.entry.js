...
const WppCard = class {
  constructor(hostRef) {
    ...
    this.onBlur = (event) => {
      this.focusType = this.getUpdatedFocusInfo('card', common.FOCUS_TYPE.NONE);
      this.focusType = this.getUpdatedFocusInfo('icon', common.FOCUS_TYPE.NONE);
      this.wppBlur.emit(event);
    };
    this.onMouseDown = () => {
      this.focusType = this.getUpdatedFocusInfo('card', common.FOCUS_TYPE.MOUSE);
      this.focusType = this.getUpdatedFocusInfo('icon', common.FOCUS_TYPE.MOUSE);
    ...
    this.cardCssClasses = () => ({
      ...
      'tab-focus': this.focusType.card === common.FOCUS_TYPE.TAB && this.focusType.icon !== common.FOCUS_TYPE.TAB,
  ...
  render() {
    ...
    return (index.h(index.Host, { onMouseEnter: () => this.updateComponentState({ hover: true }), onMouseLeave: () => this.updateComponentState({ hover: false }), onMouseUp: () => this.updateComponentState({ active: false }), onMouseDown: this.onMouseDown, onKeyUp: (event) => this.onKeyUp(event, 'card'), onClick: this.onClick, onFocus: this.onFocus, onBlur: this.onBlur, "aria-disabled": this.disabled, "aria-checked": this.checked, "aria-hidden": this.disabled ? 'true' : null, htmlFor: this.name, exportparts: "card, header-outer-wrapper, header-wrapper radio, checkbox, actions-wrapper", class: this.hostCssClasses(), tabIndex: this.disabled ? -1 : this.checkTabIndex() }, index.h("div", { class: this.cardCssClasses(), part: "card" }, index.h("div", { class: this.headerWrapperCssClasses(), part: "header-outer-wrapper" }, index.h(WrappedSlot.WrappedSlot, { name: "header", wrapperClass: this.headerCssClasses(), onSlotchange: this.updateSlotData, part: "header" }), this.withRadioOrCheckbox && (index.h(index.Fragment, null, this.type === 'single' && (index.h("wpp-radio-v2-22-0", { class: "radio", internalState: displayState, name: this.name, checked: this.checked, disabled: this.disabled, index: -1, part: "radio" })), this.type === 'multiple' && (index.h("wpp-checkbox-v2-22-0", { class: "checkbox", internalState: displayState, name: this.name, checked: this.checked, disabled: this.disabled, index: -1, part: "checkbox" })))), index.h(WrappedSlot.WrappedSlot, { name: "actions", part: "actions", wrapperClass: this.actionsCssClasses(), onSlotchange: this.updateSlotData, onBlur: this.onBlur, tabIndex: 0, onKeyUp: (event) => this.onKeyUp(event, 'icon') })), index.h("slot", null))));
  }
