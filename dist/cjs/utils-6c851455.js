'use strict';

const index = require('./index-ecf423ba.js');
const utils = require('./utils-9c925efe.js');

const iconsList = [
  {
    title: 'Actions',
    groups: [
      {
        title: 'Content Actions',
        icons: [
          {
            name: 'add',
          },
          {
            name: 'add-circle',
          },
          {
            name: 'approvals',
          },
          {
            name: 'archive',
          },
          {
            name: 'archive-multiple',
          },
          {
            name: 'attach',
          },
          {
            name: 'auto-sum',
          },
          {
            name: 'available-update',
          },
          {
            name: 'bookmark',
          },
          {
            name: 'bookmark-add',
          },
          {
            name: 'bookmark-filled',
          },
          {
            name: 'close',
          },
          {
            name: 'colour',
          },
          {
            name: 'compose',
          },
          {
            name: 'copy',
          },
          {
            name: 'data-transfer',
          },
          {
            name: 'data-view-cards',
          },
          {
            name: 'data-view-list',
          },
          {
            name: 'download',
          },
          {
            name: 'drag',
          },
          {
            name: 'edit',
          },
          {
            name: 'eraser',
          },
          {
            name: 'export',
          },
          {
            name: 'external-link',
          },
          {
            name: 'eye-on',
          },
          {
            name: 'eye-off',
          },
          {
            name: 'eyedropper',
          },
          {
            name: 'favourites',
          },
          {
            name: 'favourites-filled',
          },
          {
            name: 'fill',
          },
          {
            name: 'flag-off',
          },
          {
            name: 'flag-on',
          },
          {
            name: 'hand-draw',
          },
          {
            name: 'hand-off',
          },
          {
            name: 'hand-on',
          },
          {
            name: 'import',
          },
          {
            name: 'install-update',
          },
          {
            name: 'like-off',
          },
          {
            name: 'like-on',
          },
          {
            name: 'link',
          },
          {
            name: 'lock-off',
          },
          {
            name: 'lock-on',
          },
          {
            name: 'math-formula',
          },
          {
            name: 'move',
          },
          {
            name: 'pair',
          },
          {
            name: 'paste',
          },
          {
            name: 'pen',
          },
          {
            name: 'pending',
          },
          {
            name: 'pin',
          },
          {
            name: 'pinned',
          },
          {
            name: 'print',
          },
          {
            name: 'refresh',
          },
          {
            name: 'refresh-dashed',
          },
          {
            name: 'reject',
          },
          {
            name: 'remove',
          },
          {
            name: 'remove-circle',
          },
          {
            name: 'repeat-off',
          },
          {
            name: 'repeat-on',
          },
          {
            name: 'reset',
          },
          {
            name: 'reset-dashed',
          },
          {
            name: 'scan',
          },
          {
            name: 'search',
          },
          {
            name: 'select-object',
          },
          {
            name: 'share',
          },
          {
            name: 'shut-down',
          },
          {
            name: 'sync',
          },
          {
            name: 'translate',
          },
          {
            name: 'trash',
          },
          {
            name: 'undo',
          },
          {
            name: 'unlink',
          },
          {
            name: 'unpinned',
          },
          {
            name: 'redo',
          },
          {
            name: 'upload',
          },
          {
            name: 'zoom-in',
          },
          {
            name: 'zoom-out',
          },
        ],
      },
      {
        title: 'Filter',
        icons: [
          {
            name: 'filter',
          },
          {
            name: 'sort',
          },
          {
            name: 'sort-decrease',
          },
          {
            name: 'sort-increase',
          },
          {
            name: 'tune',
          },
        ],
      },
      {
        title: 'Media Actions',
        icons: [
          {
            name: 'cut',
          },
          {
            name: 'dislike',
          },
          {
            name: 'like',
          },
          {
            name: 'next',
          },
          {
            name: 'pause',
          },
          {
            name: 'play',
          },
          {
            name: 'previous',
          },
          {
            name: 'ratio',
          },
          {
            name: 'record',
          },
          {
            name: 'record-stop',
          },
          {
            name: 'stop',
          },
          {
            name: 'speaker',
          },
          {
            name: 'speaker-off',
          },
        ],
      },
      {
        title: 'Settings',
        icons: [
          {
            name: 'gear',
          },
          {
            name: 'service',
          },
        ],
      },
    ],
  },
  {
    title: 'Arrows',
    groups: [
      {
        title: 'Arrows',
        icons: [
          {
            name: 'arrow',
            directions: ['up', 'down', 'right', 'left'],
          },
          {
            name: 'bounce',
          },
          {
            name: 'chevron',
            directions: ['up', 'down', 'right', 'left'],
          },
          {
            name: 'double-chevron',
            directions: ['top', 'down', 'right', 'left'],
          },
          {
            name: 'enter',
          },
          {
            name: 'forward',
          },
          {
            name: 'trend-ascend',
          },
          {
            name: 'trend-descent',
          },
        ],
      },
      {
        title: 'Table sort',
        icons: [
          {
            name: 'table-sort',
          },
          {
            name: 'table-sort-hover',
          },
          {
            name: 'table-sort-pressed',
          },
          {
            name: 'table-sort-asc',
          },
          {
            name: 'table-sort-asc-hover',
          },
          {
            name: 'table-sort-asc-pressed',
          },
          {
            name: 'table-sort-desc',
          },
          {
            name: 'table-sort-desc-hover',
          },
          {
            name: 'table-sort-desc-pressed',
          },
        ],
      },
    ],
  },
  {
    title: 'Communication',
    groups: [
      {
        title: 'Communications',
        icons: [
          {
            name: 'about',
          },
          // {
          //   name: 'ask',
          // },
          {
            name: 'assistant',
          },
          {
            name: 'bot',
          },
          {
            name: 'channel',
          },
          {
            name: 'chat-message',
          },
          {
            name: 'chat-message-off',
          },
          {
            name: 'chat-video',
          },
          {
            name: 'comment',
          },
          {
            name: 'comment-off',
          },
          {
            name: 'communication',
          },
          {
            name: 'support-chat',
          },
          {
            name: 'help',
          },
          {
            name: 'inbox',
          },
          {
            name: 'info', // ?
          },
          {
            name: 'mail',
          },
          {
            name: 'mail-add',
          },
          {
            name: 'mail-all-read',
          },
          {
            name: 'mail-blocked',
          },
          {
            name: 'mail-copy',
          },
          {
            name: 'mail-off',
          },
          {
            name: 'mail-read',
          },
          {
            name: 'meet',
          },
          {
            name: 'mention',
          },
          {
            name: 'note',
          },
          {
            name: 'notification',
          },
          {
            name: 'notification-off',
          },
          {
            name: 'notification-new',
          },
          {
            name: 'notification-pause',
          },
          {
            name: 'phone',
          },
          {
            name: 'qa',
          },
          {
            name: 'reply',
          },
          {
            name: 'reply-all',
          },
          {
            name: 'send',
          },
          {
            name: 'send-off',
          },
          {
            name: 'subscribe',
          },
          {
            name: 'support',
          },
        ],
      },
    ],
  },
  {
    title: 'Content',
    groups: [
      {
        title: 'Calendar',
        icons: [
          {
            name: 'calendar',
          },
          {
            name: 'calendar-3-days',
          },
          {
            name: 'calendar-add',
          },
          {
            name: 'calendar-date',
          },
          {
            name: 'calendar-dismiss',
          },
          {
            name: 'calendar-error',
          },
          {
            name: 'calendar-month',
          },
        ],
      },
      {
        title: 'Charts',
        icons: [
          {
            name: 'bar-chart',
          },
          {
            name: 'bar-horizontal',
          },
          {
            name: 'data-area',
          },
          {
            name: 'data-funnel',
          },
          {
            name: 'data-histogram',
          },
          {
            name: 'data-line',
          },
          {
            name: 'data-scatter',
          },
          {
            name: 'data-sunburst',
          },
          {
            name: 'data-treemap',
          },
          {
            name: 'data-trending',
          },
          {
            name: 'data-usage',
          },
          {
            name: 'data-waterfall',
          },
          {
            name: 'data-whisker',
          },
          {
            name: 'database',
          },
          {
            name: 'decrease',
          },
          {
            name: 'graph',
          },
          {
            name: 'increase',
          },
          {
            name: 'neutral-trading',
          },
          {
            name: 'pie-chart',
          },
          {
            name: 'ranking',
          },
          {
            name: 'statistic-document',
          },
        ],
      },
      {
        title: 'Cloud',
        icons: [
          {
            name: 'cloud-dismiss',
          },
          {
            name: 'cloud-success',
          },
          {
            name: 'cloud-sync',
          },
          {
            name: 'cloud-upload',
          },
          {
            name: 'data-cloud-off',
          },
          {
            name: 'data-cloud-on',
          },
        ],
      },
      {
        title: 'Content',
        icons: [
          {
            name: 'accessibility',
          },
          {
            name: 'ai',
          },
          {
            name: 'board',
          },
          {
            name: 'branch',
          },
          {
            name: 'branch-review',
          },
          {
            name: 'branch-request',
          },
          {
            name: 'cart',
          },
          {
            name: 'campaign',
          },
          {
            name: 'clock',
          },
          {
            name: 'currency',
          },
          {
            name: 'dark-theme',
          },
          {
            name: 'design',
          },
          {
            name: 'diversity',
          },
          {
            name: 'draft',
          },
          {
            name: 'experiment',
          },
          {
            name: 'globe',
          },
          {
            name: 'gps',
          },
          {
            name: 'grid',
          },
          {
            name: 'history',
          },
          {
            name: 'home',
          },
          {
            name: 'home-checkmark',
          },
          {
            name: 'hub',
          },
          {
            name: 'idea',
          },
          {
            name: 'key',
          },
          {
            name: 'keyboard',
          },
          {
            name: 'launch',
          },
          {
            name: 'layer',
          },
          {
            name: 'location',
          },
          {
            name: 'magic-wand',
          },
          {
            name: 'map',
          },
          {
            name: 'money',
          },
          {
            name: 'number',
          },
          {
            name: 'hash',
          },
          {
            name: 'organization',
          },
          {
            name: 'pitch',
          },
          {
            name: 'shield',
          },
          {
            name: 'shield-success',
          },
          {
            name: 'shield-dismiss',
          },
          {
            name: 'shield-error',
          },
          {
            name: 'styleguide',
          },
          {
            name: 'sub-items',
          },
          {
            name: 'target',
          },
          {
            name: 'task-list',
          },
          {
            name: 'task-list-square',
          },
          {
            name: 'twenty-four-hours',
          },
          {
            name: 'workshop',
          },
          {
            name: 'fluid-workflow',
          },
          {
            name: 'carousel',
          },
          {
            name: 'iframe',
          },
          {
            name: 'CTA',
          },
          {
            name: 'accordion',
          },
          {
            name: 'segmented-control',
          },
          {
            name: 'action',
          },
        ],
      },
      {
        title: 'Currencies',
        icons: [
          {
            name: 'btc',
          },
          {
            name: 'currency',
          },
          {
            name: 'dollar',
          },
          {
            name: 'eth',
          },
          {
            name: 'euro',
          },
          {
            name: 'money',
          },
          {
            name: 'pound',
          },
          {
            name: 'rupee',
          },
          {
            name: 'yen',
          },
          {
            name: 'yuan',
          },
        ],
      },
      {
        title: 'Emoji',
        icons: [
          {
            name: 'angry',
          },
          {
            name: 'laugh',
          },
          {
            name: 'multiple-emotions',
          },
          {
            name: 'neutral',
          },
          {
            name: 'sad',
          },
          {
            name: 'sad-slightly',
          },
          {
            name: 'smile',
          },
          {
            name: 'smile-slightly',
          },
          {
            name: 'sparkle',
          },
          {
            name: 'surprise',
          },
          {
            name: 'with-hand',
          },
        ],
      },
      {
        title: 'Ecology',
        icons: [
          {
            name: 'leaf',
          },
          {
            name: 'eco',
          },
          {
            name: 'air-pollution',
          },
          {
            name: 'factory',
          },
          {
            name: 'recycle',
          },
          {
            name: 'nuclear-danger',
          },
        ],
      },
      {
        title: 'Files',
        icons: [
          {
            name: 'add-document',
          },
          {
            name: 'add-folder',
          },
          {
            name: 'app-folder',
          },
          {
            name: 'collection',
          },
          {
            name: 'document',
          },
          {
            name: 'document-blocked',
          },
          {
            name: 'document-error',
          },
          {
            name: 'export-file',
          },
          {
            name: 'file',
          },
          {
            name: 'file-css',
          },
          {
            name: 'file-js',
          },
          {
            name: 'file-pdf',
          },
          {
            name: 'file-zip',
          },
          {
            name: 'folder',
          },
          {
            name: 'folder-blocked',
          },
          {
            name: 'folder-link',
          },
          {
            name: 'folder-open',
          },
          {
            name: 'gallery',
          },
          {
            name: 'library',
          },
          {
            name: 'spreadsheet',
          },
        ],
      },
      {
        title: 'Shapes',
        icons: [
          {
            name: 'circle',
          },
          {
            name: 'cube',
          },
          {
            name: 'diamond',
          },
          {
            name: 'pentagon',
          },
          {
            name: 'rectangle',
          },
          {
            name: 'rhombus',
          },
          {
            name: 'shapes',
          },
          {
            name: 'square',
          },
          {
            name: 'square-hint',
          },
          {
            name: 'triangle',
          },
        ],
      },
      {
        title: 'Table',
        icons: [
          {
            name: 'table-default',
          },
          {
            name: 'table-dismiss',
          },
          {
            name: 'table-edit',
          },
          {
            name: 'table-link',
          },
          {
            name: 'table-simple',
          },
        ],
      },
    ],
  },
  {
    title: 'Media',
    groups: [
      {
        title: 'Media',
        icons: [
          {
            name: 'add-app',
          },
          {
            name: 'app',
          },
          {
            name: 'app-store',
          },
          {
            name: 'bluetooth-off',
          },
          {
            name: 'bluetooth-on',
          },
          {
            name: 'book',
          },
          {
            name: 'book-contacts',
          },
          {
            name: 'book-faq',
          },
          {
            name: 'book-information',
          },
          {
            name: 'book-search',
          },
          {
            name: 'camera-add',
          },
          {
            name: 'camera-off',
          },
          {
            name: 'camera-on',
          },
          {
            name: 'camera-switch',
          },
          {
            name: 'caption-off',
          },
          {
            name: 'caption-on',
          },
          {
            name: 'cast',
          },
          {
            name: 'cellular-data-3g',
          },
          {
            name: 'cellular-data-4g',
          },
          {
            name: 'cellular-data-5g',
          },
          {
            name: 'cellular-data-off',
          },
          {
            name: 'cellular-data-on',
          },
          {
            name: 'cookies',
          },
          {
            name: 'extension',
          },
          {
            name: 'fingerprint',
          },
          {
            name: 'gif',
          },
          {
            name: 'hd',
          },
          {
            name: 'hdr',
          },
          {
            name: 'headphones',
          },
          {
            name: 'icons',
          },
          {
            name: 'image',
          },
          {
            name: 'image-off',
          },
          {
            name: 'java-script',
          },
          {
            name: 'live',
          },
          {
            name: 'mic-blocked',
          },
          {
            name: 'mic-off',
          },
          {
            name: 'mic-on',
          },
          {
            name: 'movie',
          },
          {
            name: 'music',
          },
          {
            name: 'premium',
          },
          {
            name: 'qr-code',
          },
          {
            name: 'remove-app',
          },
          {
            name: 'rss',
          },
          {
            name: 'speaker-bluetooth',
          },
          {
            name: 'speaker-mute',
          },
          {
            name: 'sticker',
          },
          {
            name: 'symbols',
          },
          {
            name: 'video-blocked',
          },
          {
            name: 'video-clip',
          },
          {
            name: 'video-off',
          },
          {
            name: 'video-on',
          },
          {
            name: 'wifi-off',
          },
          {
            name: 'wifi-on',
          },
          {
            name: 'wifi-warning',
          },
        ],
      },
      {
        title: 'Media Channels and Devices',
        icons: [
          { name: 'desktop' },
          { name: 'laptop' },
          { name: 'mobile' },
          { name: 'outdoor-media' },
          { name: 'radio' },
          { name: 'newspaper' },
          { name: 'smartwatch' },
          { name: 'social-media' },
          { name: 'tablet' },
          { name: 'tv' },
          { name: 'vr' },
        ],
      },
    ],
  },
  {
    title: 'Profile',
    groups: [
      {
        title: 'Profile',
        icons: [
          {
            name: 'admin',
          },
          {
            name: 'incognito',
          },
          {
            name: 'login',
          },
          {
            name: 'logout',
          },
          {
            name: 'owner',
          },
          {
            name: 'people',
          },
          {
            name: 'private-account',
          },
          {
            name: 'user-starred',
          },
          {
            name: 'user',
          },
          {
            name: 'user-add',
          },
          {
            name: 'user-block',
          },
          {
            name: 'user-chat',
          },
          {
            name: 'user-search',
          },
          {
            name: 'user-tag',
          },
        ],
      },
    ],
  },
  {
    title: 'Status',
    groups: [
      {
        title: 'Status',
        icons: [
          {
            name: 'available-checkmark',
          },
          {
            name: 'done',
          },
          {
            name: 'error',
          },
          {
            name: 'success',
          },
          {
            name: 'warning',
          },
          {
            name: 'incomplete',
          },
          {
            name: 'info-message',
          },
        ],
      },
    ],
  },
  {
    title: 'System',
    groups: [
      {
        title: 'Controls',
        icons: [
          {
            name: 'dash',
          },
          {
            name: 'tick',
          },
        ],
      },
      {
        title: 'Menu',
        icons: [
          {
            name: 'more',
          },
          {
            name: 'navigation-menu',
          },
        ],
      },
    ],
  },
  {
    title: 'Tools',
    groups: [
      {
        title: 'Align',
        icons: [
          {
            name: 'editor-object-alignment-center',
          },
          {
            name: 'object-alignment',
            directions: ['top', 'bottom', 'right', 'left'],
          },
          {
            name: 'object-alignment-center',
            directions: ['horizontal', 'vertical'],
          },
          {
            name: 'text-alignment-center',
          },
          {
            name: 'text-alignment-distribute',
          },
          {
            name: 'text-alignment-distribute-vertical',
          },
          {
            name: 'text-alignment-justify',
          },
          {
            name: 'text-alignment-justify-low',
          },
          {
            name: 'text-alignment-left',
          },
          {
            name: 'text-alignment-right',
          },
        ],
      },
      {
        title: 'Resize & Scale',
        icons: [
          {
            name: 'autofit',
            directions: ['up', 'down'],
          },
          {
            name: 'autofit-height',
          },
          {
            name: 'autofit-height-dotted',
          },
          {
            name: 'autofit-width',
          },
          {
            name: 'autofit-width-dotted',
          },
          {
            name: 'scale-bottom',
            directions: ['left', 'right'],
          },
          {
            name: 'collapse',
          },
          {
            name: 'enlarge',
          },
          {
            name: 'expand',
          },
          {
            name: 'fit-height',
          },
          {
            name: 'fit-width',
          },
          {
            name: 'fullscreen',
          },
          {
            name: 'fullscreen-minimise',
          },
          {
            name: 'maximise',
            directions: ['vertical'],
          },
          {
            name: 'minimise',
            directions: ['vertical'],
          },
          {
            name: 'resize',
            directions: ['horizontal', 'vertical'],
          },
          {
            name: 'resize-large',
          },
          {
            name: 'resize-small',
          },
          {
            name: 'scale',
          },
        ],
      },
      {
        title: 'Text Formatting',
        icons: [
          {
            name: 'blockquote',
          },
          {
            name: 'bold',
          },
          {
            name: 'case',
          },
          {
            name: 'code-view',
          },
          {
            name: 'edit-text',
          },
          {
            name: 'editor-lowercase',
          },
          {
            name: 'editor-title',
          },
          {
            name: 'editor-uppercase',
          },
          {
            name: 'effects',
          },
          {
            name: 'font',
          },
          {
            name: 'h1',
          },
          {
            name: 'h2',
          },
          {
            name: 'h3',
          },
          {
            name: 'italic',
          },
          {
            name: 'ordered-list',
          },
          {
            name: 'strike-through',
          },
          {
            name: 'text',
          },
          {
            name: 'underline',
          },
          {
            name: 'unordered-list',
          },
          {
            name: 'float-left',
          },
          {
            name: 'float-center',
          },
          {
            name: 'float-right',
          },
        ],
      },
      {
        title: 'Transform',
        icons: [
          {
            name: 'border',
            directions: ['top', 'bottom', 'right', 'left'],
          },
          {
            name: 'border-all',
          },
          {
            name: 'border-bottom-double',
          },
          {
            name: 'border-left-right',
          },
          {
            name: 'border-none',
          },
          {
            name: 'border-outside',
          },
          {
            name: 'border-outside-thick',
          },
          {
            name: 'border-top-bottom',
          },
          {
            name: 'border-top-bottom-double',
          },
          {
            name: 'border-top-bottom-thick',
          },
          {
            name: 'crop',
          },
          {
            name: 'flip',
            directions: ['horizontal', 'vertical'],
          },
          {
            name: 'merge',
          },
          {
            name: 'orientation',
          },
          {
            name: 'padding',
            directions: ['top', 'bottom', 'right', 'left'],
          },
          {
            name: 'rotate',
            directions: ['right', 'left'],
          },
          {
            name: 'rotate-clockwise',
          },
          {
            name: 'rotate-counterclockwise',
          },
          {
            name: 'split',
          },
          {
            name: 'wrap-off',
          },
          {
            name: 'wrap-on',
          },
          {
            name: 'columns-three',
          },
          {
            name: 'columns-two',
          },
          {
            name: 'columns-one',
          },
        ],
      },
    ],
  },
  {
    title: 'POIs',
    groups: [
      {
        title: 'Food and Drink',
        icons: [
          {
            name: 'cafe',
          },
          {
            name: 'restaurant',
          },
          {
            name: 'night-life',
          },
          {
            name: 'bar',
          },
        ],
      },
      {
        title: 'Retail',
        icons: [
          {
            name: 'shopping',
          },
          {
            name: 'supermarket',
          },
          {
            name: 'pharmacy',
          },
        ],
      },
      {
        title: 'Services',
        icons: [
          {
            name: 'bank',
          },
          {
            name: 'atm',
          },
          {
            name: 'gas-station',
          },
          {
            name: 'lodging',
          },
          {
            name: 'post',
          },
          {
            name: 'hospital',
          },
          {
            name: 'police',
          },
        ],
      },
      {
        title: 'Entertainment',
        icons: [
          {
            name: 'tourism',
          },
          {
            name: 'cinema',
          },
          {
            name: 'zoo',
          },
          {
            name: 'sport',
          },
          {
            name: 'theatre',
          },
          {
            name: 'event',
          },
        ],
      },
      {
        title: 'Transport',
        icons: [
          {
            name: 'bus',
          },
          {
            name: 'train',
          },
          {
            name: 'bicycle',
          },
          {
            name: 'airplane',
          },
          {
            name: 'ship',
          },
          {
            name: 'parking',
          },
        ],
      },
      {
        title: 'Generic',
        icons: [
          {
            name: 'library-building',
          },
          {
            name: 'school',
          },
          {
            name: 'business',
          },
          {
            name: 'outdoor',
          },
        ],
      },
    ],
  },
];

const convertMBToBytes = (size) => size * 1024 ** 2;
const getExtension = (filename = '') => `.${filename.split('.').pop()}`;
const getExtensionsList = (acceptConfig) => Object.entries(acceptConfig).reduce((acc, [_, extensions]) => [...acc, ...extensions], []);
const getBaseName = (fileName = '') => fileName.split('.')[0];
const renameFile = (file, newFileName) => new File([file], newFileName, { type: file.type });
const modifyPropertiesOnFile = (file, properties) => new File([file], file.name, { ...properties });
//  * WPPOPENDS-512 WPPOPENDS-311
// Extracting all the icon names from iconsList, to force register and prevent tree shaking during build
const getIconNames = () => {
  const iconNames = [];
  iconsList.forEach(category => {
    category.groups.forEach(group => {
      group.icons.forEach(icon => {
        iconNames.push(`wpp-icon-${icon.name}`);
      });
    });
  });
  return iconNames;
};
// Render all icons dynamically using transformToVersionedTag
const renderIcons = () => {
  const iconNames = getIconNames();
  return iconNames.map(iconName => {
    const versionedTag = utils.transformToVersionedTag(iconName);
    return index.h(versionedTag, { 'data-fake': 'prevent-tree-shaking' }); // Dynamically create each icon component
  });
};

exports.convertMBToBytes = convertMBToBytes;
exports.getBaseName = getBaseName;
exports.getExtension = getExtension;
exports.getExtensionsList = getExtensionsList;
exports.modifyPropertiesOnFile = modifyPropertiesOnFile;
exports.renameFile = renameFile;
exports.renderIcons = renderIcons;
