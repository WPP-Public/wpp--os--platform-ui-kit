...
function renderDropdownPillsComponent() {
  ...
  const showMoreLessRender = (label) => (index.h("wpp-action-button-v4-3-0", { "data-testid": "wpp-autocomplete-show-btn", class: "nowrap", variant: "secondary", onClick: this.handleShowMoreLessClick }, label));
  const renderPillComponent = (option, ndx, isTransparentPill = false) => (index.h("wpp-pill-v4-3-0", { ref: ref => {
      if (ref)
  ...
  return (index.h(index.Fragment, null,
    ...
    isNeedDivider && index.h("wpp-divider-v4-3-0", { class: "nothing-found-divider" })));
}
...
function renderPlaceholderTextComponent() {
  ...
  return (index.h(index.Fragment, null,
    index.h("wpp-typography-v4-3-0", { ref: ref => (this.inputPlaceholderRef = ref), type: "s-body", class: inputPlaceholderCssClasses() }, this.placeholderText),
    index.h("wpp-typography-v4-3-0", { ref: ref => (this.hiddenInputPlaceholderRef = ref), role: "presentation", type: "s-body", class: "hidden-input-placeholder" }),
    this.value.length > 1 && (index.h("wpp-typography-v4-3-0", { class: hiddenCountCssClasses(), type: "s-body" },
      "+ ",
  ...
  const renderListOptions = () => {
    ...
    return (index.h(index.Fragment, null,
      this.internalList.map((item, ndx) => {
        ...
        return (index.h("wpp-list-item-v4-3-0", { ref: el => (this.listItemsRefs[ndx] = el), onWppChangeListItem: this.handleClickListItem, key: convertValueToKey(item.value), isDarkTheme: this.isDarkTheme, ...rest, id: item.id !== undefined ? `${utils$1.LIB_COMPONENTS_PREFIX}list-item-${item.id}` : undefined, role: "option" },
          index.h("span", { slot: "label" }, label),
          slots && utils$1.renderSlotsInListItem(slots, Boolean(label)).map((slotNode) => slotNode)));
      }),
      this.isInfiniteLoading && (index.h("div", { class: "wpp-dropdown-infinite-loader" },
        index.h("wpp-spinner-v4-3-0", null)))));
  };
  const renderSuggestionOptions = () => {
    this.suggestionsItemsRefs = [];
    return (index.h(index.Fragment, null,
      index.h("wpp-typography-v4-3-0", { role: "presentation", type: "s-strong", class: "suggestions-heading" }, this._locales.suggestionTitle),
      this.componentSuggestions?.map((suggestion, ndx) => {
        const { slots, checked, label, ...restProps } = suggestion;
        const isChecked = checked || utils$1.isSelected(this.value, suggestion, this.getItemKey);
        return (index.h("wpp-list-item-v4-3-0", { ref: el => (this.suggestionsItemsRefs[ndx] = el), onWppChangeListItem: this.handleClickListItem, key: convertValueToKey(suggestion.value), ...restProps, id: suggestion.id !== undefined ? `${utils$1.LIB_COMPONENTS_PREFIX}list-item-${suggestion.id}` : undefined, checked: isChecked, isDarkTheme: this.isDarkTheme, class: { 'suggestion-item': true, 'last-item': ndx === this.componentSuggestions.length - 1 }, role: "option" },
          index.h("span", { slot: "label" }, label),
          slots && utils$1.renderSlotsInListItem(slots, Boolean(label)).map((slotNode) => slotNode)));
      })));
  };
  if (this.loading) {
    return (index.h("wpp-list-item-v4-3-0", { isDarkTheme: this.isDarkTheme, role: "status", "non-interactive": true },
      index.h("wpp-spinner-v4-3-0", { slot: "left" }),
      index.h("span", { slot: "label" }, this._locales.loading)));
  }
  if (!this.visibleOptionsLength && this.searchText.length > 0) {
    return (index.h("wpp-list-item-v4-3-0", { isDarkTheme: this.isDarkTheme, labelTypography: { color: 'var(--wpp-grey-color-700)', type: 's-body' }, nonInteractive: true },
      index.h("wpp-typography-v4-3-0", { type: "s-body", class: "nothing-found", slot: "label" }, this._locales.nothingFound)));
  }
...
function renderExtendedSelectedValuesComponent() {
  return (index.h("div", { class: "selected-values" }, this.extendedSelectedValues?.map(el => (index.h("wpp-pill-v4-3-0", { label: el.label, type: "display", onWppClose: event => this.handleClickListItem({
      ...event,
...
function renderCreateNewOptionComponent() {
  ...
  return (index.h("div", { class: "wpp-dropdown-actions" },
    index.h("wpp-divider-v4-3-0", null),
    index.h("wpp-list-item-v4-3-0", { onClick: handleCreateNewOptionClick },
      index.h("wpp-typography-v4-3-0", { type: "s-strong", class: "wpp-create-new-option", slot: "label" }, this._locales.createNewElement(this.searchText)))));
}
...
const WppAutocomplete = class {
  constructor(hostRef) {
    ...
    this.themeSubscription = subscribeToTheme.themeSubscriptionController(() => this.dropdownRef, (theme) => {
    ...
    this.onClickListItemMultiple = (listItemValue) => {
      const already = utils$1.isSelected(this.value, listItemValue, this.getItemKey);
      let next;
      if (already) {
    ...
    this.checkListAgainstValue = () => {
      const next = this.internalList?.map(item => ({
        ...item,
        checked: utils$1.isSelected(this.value, item, this.getItemKey),
      }));
      ...
      const _selectedOptions = utils$1.selectedOptionsByOrder(mergedSource, this.value, this.getItemKey);
