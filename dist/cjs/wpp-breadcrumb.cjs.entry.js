...
const WppBreadcrumb = class {
  ...
  createItemElement(item, isActive = false) {
    ...
    if (isActive) {
      return (index.h("wpp-tooltip-v3-4-0", { text: item.label, part: "item-tooltip", config: {
          onShow: () => {
      ...
      return (index.h("wpp-tooltip-v3-4-0", { text: item.label, part: "item-tooltip" }, index.h("span", { class: "item", onClick: this.createRouteChangeTrigger(item), tabIndex: 0, part: "item-text" }, truncatedLabel)));
    }
    else if (item.label.length > this.maxLabelLength) {
      return (index.h("wpp-tooltip-v3-4-0", { text: item.label, part: "item-tooltip" }, index.h("a", { href: item.path, class: "item", onClick: this.createRouteChangeTrigger(item), tabIndex: 0, part: "item-text" }, truncatedLabel)));
    }
  ...
  createMenuElement(item) {
    if (this.nativeLink) {
      return (index.h("wpp-list-item-v3-4-0", { key: utils.uuidv4(), class: "link", linkConfig: { href: item.path }, part: "menu-item" }, index.h("span", { slot: "label", part: "menu-item-label" }, item.label)));
    }
    else {
      return (index.h("wpp-list-item-v3-4-0", { key: utils.uuidv4(), class: "link", part: "menu-item", onClick: this.createRouteChangeTrigger(item) }, index.h("span", { slot: "label", part: "menu-item-label" }, item.label)));
    }
  }
  render() {
    if (this.backBtnLabel) {
      return (index.h(index.Host, { class: this.hostCssClasses(), exportparts: "icon" }, index.h("button", { class: "back", onClick: this.handleBackClick, onKeyDown: this.handleBackKeyDown, type: "button", tabIndex: 0 }, index.h("wpp-icon-chevron-v3-4-0", { class: "back-icon-chevron", part: "icon", direction: "left", size: "s" }), index.h("span", { class: "back-label" }, this.backBtnLabel))));
    }
    if (!this.rootItem) {
      return;
    }
    return (index.h(index.Host, { class: this.hostCssClasses(), exportparts: "item-tooltip, item-text, menu-item, menu-item-label, slash, menu, icon-more, slash" }, this.createItemElement(this.rootItem), this.hiddenItems.length > 0 && (index.h(index.Fragment, null, index.h("div", { class: "slash", part: "slash" }, "/"), index.h("wpp-menu-context-v3-4-0", { key: this.hiddenItemsSnapshot, class: "menu", dropdownConfig: { triggerElementWidth: false, ...this.dropdownConfig }, tabIndex: 0, part: "menu" }, index.h("wpp-icon-more-v3-4-0", { class: "menu-trigger", direction: "horizontal", slot: "trigger-element", part: "icon-more" }), index.h("div", { key: this.hiddenItemsSnapshot }, this.hiddenItems.map(item => this.createMenuElement(item)))))), this.visibleItems.map((item, index$1, items) => (index.h(index.Fragment, null, index.h("div", { class: "slash", tabIndex: -1, part: "slash" }, "/"), this.createItemElement(item, index$1 === items.length - 1))))));
  }
