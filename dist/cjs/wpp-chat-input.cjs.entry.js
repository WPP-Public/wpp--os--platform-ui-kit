...
const WppChatInput = class {
  ...
  render() {
    ...
    return (index.h(index.Host, { class: this.hostCssClasses(), size: this.size, style: { zIndex: this.zIndex.toString() }, exportparts: "chat-input-container, toast, input-area, attachments, text-input, actions-bar, left-actions, right-actions, file-item", onClick: isMinimizedS ? this.handleSizeToggle : this.handleClick }, index.h("div", { class: this.chatInputContainerClasses(), part: "chat-input-container" }, this.showToast && (index.h("wpp-toast-v3-3-1", { message: this.toastMessage, type: this.toastType, duration: TOAST_DURATION, variant: "chat", part: "toast", class: this.chatToastClasses(), onClick: event => this.handleToastClick(event) })), index.h("div", { class: this.inputAreaClasses(), ref: el => (this.inputAreaRef = el), part: "input-area" }, maximizedSorSizeM ? (index.h(index.Fragment, null, allFiles?.length > 0 && (index.h("div", { class: this.attachmentsWrapperClasses(), part: "attachments" }, allFiles.map((file, index$1) => (index.h("wpp-file-upload-item-v3-3-1", { key: index$1, file: file, format: this.mergedFileUploadConfig.format, maxLabelLength: this.mergedFileUploadConfig.maxLabelLength, currentIndex: index$1, onWppDelete: this.handleDeleteItem, onWppClick: this.handleClickItem, locales: {
        sizeError: this.mergedFileUploadConfig.locales.sizeError,
        formatError: this.mergedFileUploadConfig.locales.formatError,
      }, part: "file-item", class: this.isFileWithError(file) ? 'error' : '', onFileLoaded: this.handleFileLoaded, uploaded: !!file.uploaded }))))), index.h("textarea", { class: this.textInputClasses(), placeholder: this.placeholder, value: this.internalValue, ref: el => (this.textareaRef = el), onInput: this.handleInput, onPaste: this.handlePaste, disabled: this.disabled, onKeyDown: this.onKeyDown, part: "text-input" }))) : (index.h("div", { class: this.inputAreaWrapperClasses() }, index.h("div", { class: this.minimizedInput(), part: "minimized-input", onClick: this.handleSizeToggle }, index.h("wpp-typography-v3-3-1", { class: this.inputValue(), type: "s-body" }, this.internalValue || this.placeholder)), index.h("wpp-action-button-v3-3-1", { "data-testid": "send-icon-only-button", variant: "secondary", onClick: e => {
        e.stopPropagation();
        this.handleSend();
      }, disabled: this.isSendDisabled }, index.h("wpp-icon-send-v3-3-1", { slot: "icon-start" }))))), maximizedSorSizeM && (index.h("div", { class: this.actionsBarClasses(), part: "actions-bar" }, index.h("div", { class: this.leftActionsClasses(), part: "left-actions" }, this.enableAttach && (index.h(index.Fragment, null, index.h("wpp-action-button-v3-3-1", { "data-testid": "attach-icon-only-button", disabled: this.disabled, variant: "secondary", onClick: () => this.handleFileSelection() }, index.h("wpp-icon-attach-v3-3-1", { slot: "icon-start" })), index.h("input", { class: "file-loader", type: "file", ref: inputRef => (this.inputRef = inputRef), style: { display: 'none' }, multiple: this.mergedFileUploadConfig.multiple, onChange: this.handleChange, accept: this.getAcceptExtensions().join(), title: "" }))), this.withSelect && (index.h(WrappedSlot.WrappedSlot, { wrapperClass: this.selectClasses(), name: "select", onSlotchange: this.updateSlotData })), this.enableMic && (index.h("wpp-action-button-v3-3-1", { "data-testid": "mic-icon-only-button", variant: "secondary", disabled: this.disabled }, index.h("wpp-icon-mic-on-v3-3-1", { slot: "icon-start" })))), index.h("div", { class: this.rightActionsClasses(), part: "right-actions" }, charExceeded && (index.h("wpp-typography-v3-3-1", { class: "char-counter", type: "xs-midi" }, this.internalValue.length, "/", this.charactersLimit)), index.h("wpp-action-button-v3-3-1", { "data-testid": "send-icon-only-button", variant: "secondary", onClick: () => this.handleSend(), disabled: this.isSendDisabled }, index.h("wpp-icon-send-v3-3-1", { slot: "icon-start" }))))))));
  }
