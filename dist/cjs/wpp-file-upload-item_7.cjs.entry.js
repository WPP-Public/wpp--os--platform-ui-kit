...
const utils = require('./utils-fb205c59.js');
const utils$1 = require('./utils-06b46408.js');
...
const WppFileUploadItem = class {
  constructor(hostRef) {
    ...
    this.wppDelete = index.createEvent(this, "wppDelete", 1);
    this.wppClick = index.createEvent(this, "wppClick", 1);
    this.fileLoaded = index.createEvent(this, "fileLoaded", 7);
    ...
    this.setCurrentIcon = () => {
      if (this.isFileLoading())
        return index.h("wpp-spinner-v4-2-0", null);
      const { name } = this.file;
      if (this.isFileWithError())
        return null;
      const fileExtension = utils.getExtension(name);
      return utils.returnIconFromExtension(fileExtension, this.thumbnailUrl);
    };
    ...
    this.setCurrentError = () => {
      if (this.isFileWithError()) {
        ...
        return (index.h("div", { class: "error-wrapper" }, index.h("wpp-inline-message-v4-2-0", { class: "inline-message-error", message: currentError, type: "error", showTooltipFrom: 140, tooltipConfig: { popperOptions: { strategy: 'fixed' } } }), this.file.deletable !== false && (index.h("wpp-icon-cross-v4-2-0", { class: this.crossIconClasses(), part: "cross-icon", role: "button", tabindex: this.parentDisabled || this.file.disabled ? -1 : 0, "aria-disabled": this.parentDisabled || this.file.disabled ? 'true' : undefined, "aria-label": `Remove file ${this.file.name}`, onClick: this.handleCloseClick, onKeyDown: this.handleDeleteKeyDown, onKeyUp: this.handleDeleteKeyUp, onBlur: this.handleDeleteBlur }))));
      }
      return null;
    };
    this.getEventData = () => ({
    ...
    this.getImagePreviewUrl = () => {
      const fileExtension = utils.getExtension(this.file.name).toLowerCase();
    ...
    this.renderThumbnail = () => {
      if (this.isFileLoading())
        return index.h("wpp-spinner-v4-2-0", null);
      if (this.isFileWithError())
        return index.h("wpp-icon-error-v4-2-0", { class: "thumbnail-error-icon", "aria-hidden": "true" });
      const previewUrl = this.thumbnailUrl ?? this.getImagePreviewUrl();
      if (previewUrl)
        return index.h("img", { src: previewUrl, alt: "", class: "thumbnail-image" });
      return utils.returnIconFromExtension(utils.getExtension(this.file.name), null);
    };
    this.getChatSubtitle = () => {
      ...
      return utils.returnFileTypeLabel(utils.getExtension(this.file.name));
    };
    this.renderDeleteIcon = () => this.file.deletable !== false && (index.h("wpp-icon-cross-v4-2-0", { class: this.crossIconClasses(), part: "cross-icon", role: "button", tabindex: this.parentDisabled || this.file.disabled ? -1 : 0, "aria-disabled": this.parentDisabled || this.file.disabled ? 'true' : undefined, "aria-label": `Remove file ${this.file.name}`, onClick: this.handleCloseClick, onKeyDown: this.handleDeleteKeyDown, onKeyUp: this.handleDeleteKeyUp, onBlur: this.handleDeleteBlur }));
  ...
  componentWillLoad() {
    ...
    const fileExtension = utils.getExtension(name).toLowerCase();
  ...
  get _locales() {
    return utils$1.mergeLocales(utils.LOCALES_DEFAULTS, this.locales);
  }
  handleFileReading() {
  ...
  renderChatVariant() {
    ...
    return (index.h(index.Host, { class: this.hostCssClasses(), exportparts: "file-item, thumbnail, content, file-name, subtitle, cross-icon", onClick: this.handleClick, role: "listitem" }, index.h("div", { class: this.itemCssClasses(), part: "file-item" }, index.h("div", { class: { thumbnail: true, error: this.isFileWithError(), loading: this.isFileLoading() }, part: "thumbnail" }, this.renderThumbnail()), index.h("div", { class: "details", part: "content" }, index.h("wpp-tooltip-v4-2-0", { ref: ref => (this.tooltipRef = ref), text: this.file.name, config: {
        popperOptions: { strategy: 'fixed' },
        onShow: () => {
          if (!this.isTruncated)
            return false;
        },
      }, part: "tooltip" }, index.h("wpp-typography-v4-2-0", { ref: ref => (this.fileNameRef = ref), class: this.fileNameCssClasses(), type: "xs-midi", part: "file-name", title: this.file.name }, this.file?.name)), index.h("wpp-tooltip-v4-2-0", { text: subtitle, config: {
        popperOptions: { strategy: 'fixed' },
        onShow: () => {
      ...
      } }, index.h("wpp-typography-v4-2-0", { ref: ref => (this.subtitleRef = ref), type: "xs-body", class: { subtitle: true, 'subtitle-error': this.isFileWithError() }, part: "subtitle" }, subtitle))), this.renderDeleteIcon()), index.h("wpp-typography-v4-2-0", { ref: ref => (this.measureRef = ref), type: "xs-midi", class: "measure", "aria-hidden": "true", role: "presentation" }), index.h("wpp-typography-v4-2-0", { ref: ref => (this.subtitleMeasureRef = ref), type: "xs-body", class: "measure", "aria-hidden": "true", role: "presentation" })));
  }
  render() {
    if (this.variant === 'chat')
      return this.renderChatVariant();
    return (index.h(index.Host, { class: this.hostCssClasses(), exportparts: "file-item, wrapper, content, file-name, tooltip, loading, percentage, cross-icon", onClick: this.handleClick, role: "listitem" }, index.h("div", { class: this.itemCssClasses(), part: "file-item" }, this.setCurrentError(), index.h("div", { class: "content-wrapper", part: "wrapper" }, index.h("div", { class: this.blockCssClasses(), part: "content" }, index.h("div", { class: "icon-wrapper" }, this.setCurrentIcon()), index.h("wpp-tooltip-v4-2-0", { ref: ref => (this.tooltipRef = ref), text: this.file.name, config: {
        popperOptions: { strategy: 'fixed' },
        onShow: () => {
          if (!this.isTruncated)
            return false;
        },
      }, part: "tooltip" }, index.h("wpp-typography-v4-2-0", { ref: ref => (this.fileNameRef = ref), class: this.fileNameCssClasses(), type: "s-body", part: "file-name", title: this.file.name }, this.file?.name)), !this.isFileWithError() ? (index.h("span", { ref: ref => (this.loadingRef = ref), class: "loading", part: "loading" }, this.isFileLoading()
      ? `${this.loaded}/${this.total} ${this.measurementUnit}`
      : `${this.total} ${this.measurementUnit}`)) : (index.h("span", { class: "error-message", part: "error-message" }, this.total, " ", this.measurementUnit))), index.h("div", { class: "controls-wrapper", part: "controls" }, this.isFileLoading() && (index.h("span", { class: "percentage", part: "percentage" }, this.percentage, "%")), this.file.deletable !== false && !this.isFileWithError() && (index.h("wpp-icon-cross-v4-2-0", { class: this.crossIconClasses(), part: "cross-icon", role: "button", tabindex: this.parentDisabled || this.file.disabled ? -1 : 0, "aria-disabled": this.parentDisabled || this.file.disabled ? 'true' : undefined, "aria-label": `Remove file ${this.file.name}`, onClick: this.handleCloseClick, onKeyDown: this.handleDeleteKeyDown, onBlur: this.handleDeleteBlur, onKeyUp: this.handleDeleteKeyUp }))))), index.h("wpp-typography-v4-2-0", { ref: ref => (this.measureRef = ref), type: "s-body", class: "measure", "aria-hidden": "true", role: "presentation" })));
  }
