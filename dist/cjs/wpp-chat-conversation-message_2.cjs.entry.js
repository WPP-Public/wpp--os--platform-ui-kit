...
const utils$1 = require('./utils-7e1b79f9.js');
const _const = require('./const-09fdf30a.js');
const _const$1 = require('./const-605d2623.js');
...
const renderImageNode = (role, data, className) => {
  const img = (index.h(index.Fragment, null,
    ...
    index.h("wpp-icon-image-v4-0-0", { role: "presentation", "aria-hidden": "true" })));
  return data.linkHref ? (index.h("a", { href: data.linkHref, target: "_blank", rel: "noopener noreferrer", class: "chat-image-link" }, img)) : role === 'assistant' ? (index.h(index.Fragment, null,
    index.h("wpp-action-button-v4-0-0", { class: "image-download-button", variant: "secondary", onClick: () => handleDownload(data.href, data.alt) },
      index.h("wpp-icon-download-v4-0-0", { slot: "icon-start", "aria-hidden": "true" })),
    img)) : (img);
};
const renderToken = (token, role = 'user') => {
  switch (token.type) {
    ...
    case 'heading': {
      ...
      return (index.h("wpp-typography-v4-0-0", { type: type, tag: tag }, token.tokens ? token.tokens.map(t => renderToken(t)) : token.text));
    }
    case 'del':
      return index.h("del", null, token.tokens?.map(t => renderToken(t)));
    case 'paragraph':
      return (index.h("wpp-typography-v4-0-0", { type: "s-body", tag: "p" }, token.tokens ? token.tokens.map(t => renderToken(t)) : token.text));
    case 'blockquote':
      return index.h("blockquote", { class: "chat-quote" }, token.tokens?.map(t => renderToken(t)));
    case 'strong':
      return (index.h("wpp-typography-v4-0-0", { type: "s-strong", tag: "span" }, token.tokens?.map(t => renderToken(t))));
    case 'em':
    ...
    case 'code':
      return (index.h("pre", { class: "code-block" },
        index.h("div", { class: "code-lang" },
          ...
          index.h("wpp-icon-copy-v4-0-0", { class: "code-block-copy-icon", size: "s", color: "var(--wpp-grey-color-600)", onClick: () => {
    ...
    case 'list_item': {
      ...
      return (index.h("li", { class: `chat-list-item ${token.task ? `chat-list-item-task ${token.checked ? 'chat-list-item-task-checked' : ''}` : ''}` },
        token.task && index.h("wpp-icon-tick-v4-0-0", null),
        inlineContent.length > 0 && (index.h("wpp-typography-v4-0-0", { type: "s-body" }, inlineContent.map(t => renderToken(t)))),
        ...
        index.h("table", { class: "chat-table" },
          index.h("thead", null,
            index.h("tr", null, token.header.map((cell, i) => (index.h("th", { key: i },
              index.h("wpp-typography-v4-0-0", { type: "s-strong" }, cell.tokens?.map((t) => renderToken(t)) ?? cell.text)))))),
          index.h("tbody", null, token.rows.map((row, r) => (index.h("tr", { key: r }, row.map((cell, c) => (index.h("td", { key: c },
            index.h("wpp-typography-v4-0-0", { type: "s-body" }, cell.tokens?.map((t) => renderToken(t)) ?? cell.text)))))))))));
    }
    case 'link':
      if (token.title) {
        return (index.h("wpp-tooltip-v4-0-0", { text: token.title },
    ...
    case 'hr':
      return index.h("wpp-divider-v4-0-0", null);
    case 'text':
      return token.tokens
    ...
    case 'tag':
      ...
      return null;
    default:
      return 'text' in token ? index.h("wpp-typography-v4-0-0", { type: "s-body" }, token.text) : null;
  }
};
...
const WppChatConversationMessage = class {
  constructor(hostRef) {
    ...
    this.renderActionButton = (data) => {
      if (!data.icon)
        return null;
      return (index.h("wpp-action-button-v4-0-0", { variant: "secondary", ...data }, index.h(utils.transformToVersionedTag(data.icon), { slot: 'icon-start', part: 'icon' })));
    };
    this.renderMenuContextListItems = () => (index.h(index.Fragment, null, this.menuContextListItems.map(item => {
      ...
      return (index.h("wpp-list-item-v4-0-0", { ...rest, id: item.id !== undefined ? `${_const.LIB_COMPONENTS_PREFIX}list-item-${item.id}` : undefined, role: "option" }, index.h("span", { slot: "label" }, label), slots && utils$1.renderSlotsInListItem(slots, Boolean(label)).map((slotNode) => slotNode)));
    })));
    this.hostCssClasses = () => ({
  ...
  renderComplete() {
    ...
    if (!Array.isArray(tokens)) {
      return (index.h("wpp-typography-v4-0-0", { type: "s-body", tag: "p" }, purify.sanitize(this.finalContent)));
    }
  ...
  renderAttachments() {
    ...
    return (index.h("div", { class: "attachments" }, images.length > 0 && (index.h("div", { class: "chat-image-grid-row" }, images.map(image => (index.h("div", { class: "chat-image-grid-item chat-image-error-wrap" }, index.h("img", { src: image.url, alt: image.name, loading: "lazy", onError: (e) => {
      ...
      } })))))), otherFiles && otherFiles.length > 0 && (index.h("div", { class: "chat-file-attachments-column" }, otherFiles.map(file => (index.h("wpp-file-upload-item-v4-0-0", { file: {
        name: file.name,
        url: file.url,
        type: file.type,
        size: file.size || 0,
  ...
  render() {
    return (index.h(index.Host, { class: this.hostCssClasses() }, index.h("div", { class: this.containerCssClasses() }, this.assistantAvatarConfig && (index.h("div", { class: "avatar-wrapper" }, this.role === 'assistant' && (index.h("wpp-avatar-v4-0-0", { size: "s", icon: "wpp-icon-ai", role: "presentation", ...this.assistantAvatarConfig })))), index.h("div", { class: this.contentCssClasses() }, index.h("div", { class: this.messageCssClasses() }, this.currentStatus === 'streaming' && this.renderStreaming(), this.currentStatus === 'complete' && this.renderComplete(), this.currentStatus === 'complete' && this.attachments.length > 0 && this.renderAttachments()), this.role === 'assistant' && this.status === 'complete' && (index.h("div", { class: "actions" }, index.h("div", { class: "action-toolbar" }, this._actionButtonConfig.map(this.renderActionButton), this.menuContextListItems && (index.h("wpp-menu-context-v4-0-0", null, index.h("wpp-action-button-v4-0-0", { variant: "secondary", slot: "trigger-element" }, index.h("wpp-icon-more-v4-0-0", { slot: "icon-start", direction: "horizontal" })), this.renderMenuContextListItems()))), this.sourcesActionConfig && (index.h("div", { class: "sources-action" }, index.h("wpp-action-button-v4-0-0", { variant: "secondary", ...this.sourcesActionConfig }, this.sourcesActionConfig.text)))))), this.userAvatarConfig && (index.h("div", { class: "avatar-wrapper" }, this.role === 'user' && index.h("wpp-avatar-v4-0-0", { size: "s", role: "presentation", ...this.userAvatarConfig }))))));
  }
