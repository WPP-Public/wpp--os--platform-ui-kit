...
var handleBackspace$1 = (editor, type) => {
  ...
  const index$1 = $from.index(parentDepth);
  if (index$1 === 0) return false;
  if (parent.type === type) {
    return editor.commands.lift(type.name);
  }
  const previous = parent.child(index$1 - 1);
  ...
  tr.setSelection(index.TextSelection.create(tr.doc, targetPos));
...
function markdownLinkInputRule(config) {
  const rule = index.markInputRule({
    find: (text) => {
...
function markdownLinkPasteRule(config) {
  const rule = index.markPasteRule({
    find: (text) => {
      var _a, _b;
...
var OrderedList = index.Node3.create({
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    const attrs = index.mergeAttributes(this.options.HTMLAttributes, attributesWithoutType);
    if (start !== 1) {
      attrs.start = start;
    }
    if (type && type !== "1") {
...
function collectRescanRanges(tr, oldState, newState) {
  const ranges = [];
  if (tr.docChanged) {
    const changes = index.getChangedRanges(tr);
...
function createPlaceholderStateField({
  ...
  return {
    init(_config, state) {
      ...
      return decorations != null ? decorations : index.DecorationSet.empty;
    },
    ...
    props: {
      decorations: useResolvedPath ? ({ doc, selection }) => buildPlaceholderDecorations({ editor, options, dataAttribute, doc, selection }) : (state) => {
        ...
        if (options.showOnlyWhenEditable && !editor.isEditable) {
          return index.DecorationSet.empty;
        }
        return (_a = PLUGIN_KEY.getState(state)) != null ? _a : index.DecorationSet.empty;
      }
    }
  });
}
...
function shouldSyncDomSelection(state, editor) {
  return !state.selection.empty && !index.isNodeSelection(state.selection) && editor.isEditable;
}
