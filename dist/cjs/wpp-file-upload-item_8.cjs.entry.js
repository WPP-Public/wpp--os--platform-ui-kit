...
const _const = require('./const-fa1ee475.js');
...
const WppFileUploadItem = class {
  constructor(hostRef) {
    ...
    this.setCurrentIcon = () => {
      if (this.isFileLoading())
        return index.h("wpp-spinner-v3-6-0", null);
      const { name } = this.file;
      if (this.isFileWithError())
        return null;
      const fileExtension = _const.getExtension(name);
      return _const.returnIconFromExtension(fileExtension, this.thumbnailUrl);
    };
    ...
    this.setCurrentError = () => {
      if (this.isFileWithError()) {
        ...
        return (index.h("div", { class: "error-wrapper" }, index.h("wpp-inline-message-v3-6-0", { class: "inline-message-error", message: currentError, type: "error", showTooltipFrom: 140, tooltipConfig: { popperOptions: { strategy: 'fixed' } } }), this.file.deletable !== false && (index.h("wpp-icon-cross-v3-6-0", { class: this.crossIconClasses(), part: "cross-icon", role: "button", tabindex: this.parentDisabled || this.file.disabled ? -1 : 0, "aria-disabled": this.parentDisabled || this.file.disabled ? 'true' : undefined, "aria-label": `Remove file ${this.file.name}`, onClick: this.handleCloseClick, onKeyDown: this.handleDeleteKeyDown, onKeyUp: this.handleDeleteKeyUp, onBlur: this.handleDeleteBlur }))));
      }
      return null;
    };
    this.getEventData = () => ({
  ...
  componentWillLoad() {
    ...
    const fileExtension = _const.getExtension(name).toLowerCase();
  ...
  render() {
    return (index.h(index.Host, { class: this.hostCssClasses(), exportparts: "file-item, wrapper, content, file-name, tooltip, loading, percentage, cross-icon", onClick: this.handleClick, role: "listitem" }, index.h("div", { class: this.itemCssClasses(), part: "file-item" }, this.setCurrentError(), index.h("div", { class: "content-wrapper", part: "wrapper" }, index.h("div", { class: this.blockCssClasses(), part: "content" }, index.h("div", { class: "icon-wrapper" }, this.setCurrentIcon()), index.h("wpp-tooltip-v3-6-0", { class: this.maxLabelLength ? 'computed' : '', ref: ref => (this.tooltipRef = ref), text: this.file.name, config: {
        popperOptions: { strategy: 'fixed' },
        onShow: () => {
          if (!this.isTruncated || (this.maxLabelLength && !(this.file?.name?.length > this.maxLabelLength)))
            return false;
        },
      }, part: "tooltip" }, index.h("wpp-typography-v3-6-0", { ref: ref => (this.fileNameRef = ref), class: this.fileNameCssClasses(), type: "s-body", part: "file-name", title: this.file.name }, this.maxLabelLength ? utils.truncate(this.file.name, this.maxLabelLength, true) : this.file?.name)), !this.isFileWithError() ? (index.h("span", { ref: ref => (this.loadingRef = ref), class: "loading", part: "loading" }, this.isFileLoading()
      ? `${this.loaded}/${this.total} ${this.measurementUnit}`
      : `${this.total} ${this.measurementUnit}`)) : (index.h("span", { class: "error-message", part: "error-message" }, this.total, " ", this.measurementUnit))), index.h("div", { class: "controls-wrapper", part: "controls" }, this.isFileLoading() && (index.h("span", { class: "percentage", part: "percentage" }, this.percentage, "%")), this.file.deletable !== false && !this.isFileWithError() && (index.h("wpp-icon-cross-v3-6-0", { class: this.crossIconClasses(), part: "cross-icon", role: "button", tabindex: this.parentDisabled || this.file.disabled ? -1 : 0, "aria-disabled": this.parentDisabled || this.file.disabled ? 'true' : undefined, "aria-label": `Remove file ${this.file.name}`, onClick: this.handleCloseClick, onKeyDown: this.handleDeleteKeyDown, onBlur: this.handleDeleteBlur, onKeyUp: this.handleDeleteKeyUp }))))), index.h("wpp-typography-v3-6-0", { ref: ref => (this.measureRef = ref), type: "s-body", class: "measure", "aria-hidden": "true", role: "presentation" })));
  }
...
const WppIconDocument = class {
  ...
  static get registryIs() { return "wpp-icon-document-v3-6-0"; }
};
...
const WppIconFile = class {
  ...
  render() {
    return (index.h(WppIcon.WppIcon, { name: "wpp-icon-file", width: this.width, height: this.height, size: this.size, color: this.color }, index.h("path", { "fill-rule": "evenodd", "clip-rule": "evenodd", d: "M5.20837 1.5415C4.10374 1.5415 3.20837 2.43687 3.20837 3.5415V16.4582C3.20837 17.5628 4.10374 18.4582 5.20837 18.4582H14.7917C15.8963 18.4582 16.7917 17.5628 16.7917 16.4582V7.70817C16.7917 7.49872 16.7059 7.30932 16.5674 7.17325L11.16 1.76582C11.0239 1.62737 10.8345 1.5415 10.625 1.5415H5.20837ZM11.375 4.1022V6.45817C11.375 6.73437 11.5988 6.95817 11.875 6.95817H14.231L11.375 4.1022ZM4.70837 3.5415C4.70837 3.2653 4.93217 3.0415 5.20837 3.0415H9.87504V6.45817C9.87504 7.5628 10.7704 8.45817 11.875 8.45817H15.2917V16.4582C15.2917 16.7344 15.0679 16.9582 14.7917 16.9582H5.20837C4.93217 16.9582 4.70837 16.7344 4.70837 16.4582V3.5415Z", fill: "currentColor" })));
  }
  static get registryIs() { return "wpp-icon-file-v3-6-0"; }
};
...
const WppIconFileZip = class {
  ...
  static get registryIs() { return "wpp-icon-file-zip-v3-6-0"; }
};
...
const WppIconMusic = class {
  ...
  static get registryIs() { return "wpp-icon-music-v3-6-0"; }
};
...
const WppIconPitch = class {
  ...
  static get registryIs() { return "wpp-icon-pitch-v3-6-0"; }
};
...
const WppIconSpreadsheet = class {
  ...
  static get registryIs() { return "wpp-icon-spreadsheet-v3-6-0"; }
};
...
exports.wpp_icon_file = WppIconFile;
