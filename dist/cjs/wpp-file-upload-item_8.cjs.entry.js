...
const _const = require('./const-832e0d28.js');
...
const WppFileUploadItem = class {
  constructor(hostRef) {
    ...
    this.setCurrentIcon = () => {
      if (this.isFileLoading())
        return index.h("wpp-spinner-v3-4-0", null);
    ...
    this.setCurrentError = () => {
      if (this.isFileWithError()) {
        ...
        return (index.h("div", { class: "error-wrapper" }, index.h("wpp-inline-message-v3-4-0", { class: "inline-message-error", message: currentError, type: "error", showTooltipFrom: 140, tooltipConfig: { popperOptions: { strategy: 'fixed' } } }), this.file.deletable !== false && (index.h("wpp-icon-cross-v3-4-0", { class: this.crossIconClasses(), part: "cross-icon", role: "button", tabindex: this.parentDisabled || this.file.disabled ? -1 : 0, "aria-disabled": this.parentDisabled || this.file.disabled ? 'true' : undefined, "aria-label": `Remove file ${this.file.name}`, onClick: this.handleCloseClick, onKeyDown: this.handleDeleteKeyDown, onKeyUp: this.handleDeleteKeyUp, onBlur: this.handleDeleteBlur }))));
      }
      return null;
    };
    this.getEventData = () => ({
  ...
  render() {
    return (index.h(index.Host, { class: this.hostCssClasses(), exportparts: "file-item, wrapper, content, file-name, tooltip, loading, percentage, cross-icon", onClick: this.handleClick, role: "listitem" }, index.h("div", { class: this.itemCssClasses(), part: "file-item" }, this.setCurrentError(), index.h("div", { class: "content-wrapper", part: "wrapper" }, index.h("div", { class: this.blockCssClasses(), part: "content" }, index.h("div", { class: "icon-wrapper" }, this.setCurrentIcon()), index.h("wpp-tooltip-v3-4-0", { class: this.maxLabelLength ? 'computed' : '', ref: ref => (this.tooltipRef = ref), text: this.file.name, config: {
        popperOptions: { strategy: 'fixed' },
        onShow: () => {
          if (!this.isTruncated || (this.maxLabelLength && !(this.file?.name?.length > this.maxLabelLength)))
            return false;
        },
      }, part: "tooltip" }, index.h("wpp-typography-v3-4-0", { ref: ref => (this.fileNameRef = ref), class: this.fileNameCssClasses(), type: "s-body", part: "file-name", title: this.file.name }, this.maxLabelLength ? utils.truncate(this.file.name, this.maxLabelLength, true) : this.file?.name)), !this.isFileWithError() && (index.h("span", { ref: ref => (this.loadingRef = ref), class: "loading", part: "loading" }, this.isFileLoading()
      ? `${this.loaded}/${this.total} ${this.measurementUnit}`
      : `${this.total} ${this.measurementUnit}`))), index.h("div", { class: "controls-wrapper", part: "controls" }, this.isFileLoading() && (index.h("span", { class: "percentage", part: "percentage" }, this.percentage, "%")), this.file.deletable !== false && !this.isFileWithError() && (index.h("wpp-icon-cross-v3-4-0", { class: this.crossIconClasses(), part: "cross-icon", role: "button", tabindex: this.parentDisabled || this.file.disabled ? -1 : 0, "aria-disabled": this.parentDisabled || this.file.disabled ? 'true' : undefined, "aria-label": `Remove file ${this.file.name}`, onClick: this.handleCloseClick, onKeyDown: this.handleDeleteKeyDown, onBlur: this.handleDeleteBlur, onKeyUp: this.handleDeleteKeyUp }))))), index.h("wpp-typography-v3-4-0", { ref: ref => (this.measureRef = ref), type: "s-body", class: "measure", "aria-hidden": "true", role: "presentation" })));
  }
