...
const WppStickyBar = class {
  ...
  render() {
    return (index.h(index.Host, { class: this.hostCssClasses() }, index.h("div", { class: "container" }, this.variant === 'blank' ? (index.h("slot", { name: "content" })) : (index.h(index.Fragment, null, index.h("div", { class: "header" }, index.h("div", { class: "left-area" }, this.withBackButton && (index.h("wpp-action-button-v3-5-0", { variant: "secondary", onClick: this.handleLeftIconClick }, index.h("wpp-icon-chevron-v3-5-0", { slot: "icon-start", direction: "left" }))), index.h("wpp-typography-v3-5-0", { class: "bar-title", type: 'm-strong' }, this.barTitle)), index.h("div", { class: "right-area" }, this.buttonsList.map((buttonItem, btnIndex) => {
      if (!buttonItem)
        return null;
      if (buttonItem.variant === 'action-button') {
        return (index.h("wpp-action-button-v3-5-0", { key: buttonItem.text, onClick: () => this.handleButtonClick(btnIndex), variant: "primary" }, buttonItem.text));
      }
      return (index.h("wpp-button-v3-5-0", { size: "s", onClick: () => this.handleButtonClick(btnIndex), key: buttonItem.text, variant: buttonItem.variant }, buttonItem.text));
    }))), this.variant !== 'one-line' && (index.h("div", { class: "body" }, this.variant === 'two-lines' ? (index.h("slot", { name: "content" })) : (this.tabs.length > 0 && (index.h("wpp-tabs-v3-5-0", { size: "s", onWppChange: this.handleTabClick, value: this.currentTab }, this.tabs.map((tabItem) => (index.h("wpp-tab-v3-5-0", { size: "s", key: tabItem.value, value: tabItem.value }, tabItem.text)))))))))))));
  }
