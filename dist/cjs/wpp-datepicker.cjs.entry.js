 ...
 * @returns Normalized date range [normalizedStart, normalizedEnd]
 */
const normalizeYearRangeDates = (dates, config = { enabled: true }) => {
  if (!config.enabled || dates.length !== 2) {
    return dates;
  }
  const [start, end] = dates;
  ...
  render() {
    return (index.h(index.Host, { class: this.hostCssClasses(), exportparts: "label, datepicker-container, icon-calendar, datepicker-input, icon-cross, message, trigger-wrapper" }, this.labelConfig?.text && !this.hasTriggerSlot && (index.h("wpp-label-v4-1-0", { class: "label", htmlFor: this.name, optional: !this.required, config: this.labelConfig, tooltipConfig: this.labelTooltipConfig, part: "label" })), index.h("div", { class: this.containerClasses(), id: "container", part: "datepicker-container" }, this.hasTriggerSlot
      ...
      : [
        ...
        index.h("wpp-icon-calendar-v4-1-0", { onClick: this.handleClickCalendarIcon, class: this.iconCalendarCssClasses(), part: "icon-calendar", color: "inherit" }),
      ], index.h("div", { onBlur: this.handleBlurPortal, onFocus: () => clearTimeout(this.hideTimer), ...(this.hasPresets() ? { tabIndex: 0 } : {}), ref: ref => (this.portalRef = ref), class: this.portalClasses() }, this.hasPresets() && (index.h("div", { class: "wpp-presets-container" }, index.h("div", { class: "wpp-presets-list" }, this.presets.map((preset) => (index.h("wpp-list-item-v4-1-0", { onMouseEnter: () => this.handlePreviewPreset(preset.value), onMouseLeave: this.handleMouseLeavePreset, onWppChangeListItem: () => this.handleClickPreset(preset), class: "wpp-presets-item" }, index.h("wpp-typography-v4-1-0", { type: "s-body", slot: "label" }, preset.label))))), index.h("div", { class: "wpp-presets-footer" })))), (!!this.lastValidDate || this.inputRef?.value) && !this.hasTriggerSlot && (index.h("wpp-icon-cross-v4-1-0", { class: this.iconCrossCssClasses(), "aria-label": "Erase date", onClick: this.handleClickIconCross, onMouseDown: (e) => e.preventDefault(), part: "icon-cross" })), (this.message || this.internalMessage) && (index.h("wpp-inline-message-v4-1-0", { id: "datepicker-message", class: "inline-message", message: this.message || this.internalMessage, type: this.message ? this.messageType : this.internalMessageType, showTooltipFrom: this.maxMessageLength, tooltipConfig: this.tooltipConfig, part: "message" })))));
  }
