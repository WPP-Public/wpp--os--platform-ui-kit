...
const WppStepper = class {
  constructor(hostRef) {
    ...
    this.getStepperProps = () => {
      ...
      const stepList = this.host.querySelectorAll(utils.transformToVersionedTag('wpp-step'));
    ...
    this.calculateStepperPositionOnResize = () => {
      const stepsLength = this.host.querySelectorAll(utils.transformToVersionedTag('wpp-step')).length;
    ...
    this.setStepWidthOnResize = () => {
      ...
      this.host.querySelectorAll(utils.transformToVersionedTag('wpp-step')).forEach(step => {
    ...
    this.setStepAttributeAsIntegers = () => {
      ...
      stepList.forEach((step, index) => {
        ...
        const currentStepSubstep = step.querySelectorAll(utils.transformToVersionedTag('wpp-step'));
    ...
    this.setStepAttributeAsDecimals = () => {
      ...
      stepList.forEach((step, index) => {
        const isSubStep = step.substep;
        const hasSubStepElements = !!step.querySelector(utils.transformToVersionedTag('wpp-step'));
        const currentStepSubSteps = step.querySelectorAll(utils.transformToVersionedTag('wpp-step'));
        const hasDescription = !!step.querySelector('[slot="description"]');
        let stepValue;
        if (isSubStep) {
          if (parentStep) {
  ...
  componentDidLoad() {
    if (this.orientation === 'horizontal') {
      const stepsList = this.host.querySelectorAll(utils.transformToVersionedTag('wpp-step'));
    ...
    this.stepAttributeTimeout = setTimeout(() => {
      ...
      const stepMainList = this.hostElement?.querySelectorAll(`:scope > ${utils.transformToVersionedTag('wpp-step')}`);
  ...
  static get registryIs() { return "wpp-stepper-v4-3-0"; }
...
exports.wpp_stepper = WppStepper;
