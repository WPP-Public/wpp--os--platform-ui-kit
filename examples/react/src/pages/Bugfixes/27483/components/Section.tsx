...
const Section = ({ format }: Props) => {
  ...
  const handleDateChangeSec = (event: CustomEvent, el: 'child' | 'parent') => {
    if (el === 'child') {
      ...
      if (MAPPED_FORMATS[format]) {
        setChildValueSec(moment().format(MAPPED_FORMATS[format]))
      } else {
        setChildValueSec(moment().format(format.toUpperCase()))
      }
    }
  }
  ...
  return (
    <div className={styles.section}>
      <WppTypography className={styles.sectionTitle} type="2xl-heading">
        Date format: {format}
      </WppTypography>
