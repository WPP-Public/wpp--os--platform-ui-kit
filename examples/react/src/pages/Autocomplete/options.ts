...
export const isInfiniteLastPage = (search: string, page: number) => page > 6 - Math.round((search.length * 2) / 3)

export const getOptionLabel = (value: string | number, optionsList: BasicOption[]): string => {
...
export const generateInfiniteResults = (search: string, page: number, itemsPerPage = 20): BasicOption[] => {
  const isLastPage = isInfiniteLastPage(search, page)
  const fillPart = ((search.length + page) % 5) / 5
  const itemsCount = isLastPage ? Math.floor((fillPart * itemsPerPage) / 2 + itemsPerPage / 2) : itemsPerPage
