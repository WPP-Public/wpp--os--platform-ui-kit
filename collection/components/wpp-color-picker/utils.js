...
export const DEFAULT_THEME_SECTIONS = [
  {
    keys: ['primary'],
    title: 'Brand Primary',
  },
...
export const isValid6DigitHex = (hex) => /^#([A-Fa-f0-9]{3}|[A-Fa-f0-9]{6})$/.test(hex);
export const isValid8DigitHex = (hex) => /^#([A-Fa-f0-9]{3}){1,2}([A-Fa-f0-9]{2})?$/.test(hex);
 ...
 */
export const getColorsFromThemeOnPage = (host) => {
  const colorValues = [];
 ...
 * object represents a matrix with data for each color section.
 */
export const getColorsForSections = (themeColors) => {
  const colorValues = [];
